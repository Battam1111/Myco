...
| `myco immune --fix` | 30 次元の一貫性 lint + 機械的な自動修復。セッション終了時に自動で走る。 |
...
- 🛡️ **自己診断する**。30 次元の免疫 lint がセッション終了ごとに走る。古くなった事実、壊れた相互参照、矛盾——機械的に検出、機械的に修復。
...
Myco substrate                        │
  ...
  └── 免疫システム       30 次元の一貫性 lint、セッション終了で自動修復
  │                                   │
  └─────────── 代謝ループ ─────────────┘
