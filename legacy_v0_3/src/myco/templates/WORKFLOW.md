...
**运行方式**：`python scripts/lint_knowledge.py --project-dir .`（从项目根目录）
...
| 阶段 | 频率 | 动作 |
|------|------|------|
| Hunger Sensing | 每次会话 | `myco_hunger(execute=true)` → 自动检测 17+ 信号并修复 |
| Session Reflection | 会话结束 | `myco_reflect` → 自动创建 execution-learning note |
| Milestone Retrospective | 里程碑后 | `predict_knowledge_needs` → 分析会话历史发现知识缺口 |
| Cross-Project Distillation | 项目结束 | `evolve.py` + cross-instance skill transfer |
...
[进行中] W1 即时沉淀 | hunger 摩擦捕捉 | W3 传统手艺（如需）| W4 在线验证
[结束] ① 任务队列 ② log.md ③ 进度 ④ myco_reflect 反思 ⑤ wiki/docs 更新 ⑥ Lint（长会话）
```
