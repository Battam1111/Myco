...
archive 目录设计为 append-only 历史档案：Cross-Project Distillation 回顾、决策考古、
替代方案查询仍可读取；`detect_structural_bloat` 的非递归 glob 自然
排除 archive/。本规则的权威 craft 在
`docs/primordia/archive/primordia_compression_craft_2026-04-12.md`。
...
| 代谢阶段 | 频率 | 动作 |
|------|------|------|
| Hunger Sensing: 摩擦感知 | 每次会话 | 遇到不顺畅 → `friction` 条目到 log.md |
...
[结束] ① 任务队列 ② log.md ③ 进度 ④ Session Reflection 反思 ⑤ wiki/docs 更新 ⑥ Lint（长会话）
```
