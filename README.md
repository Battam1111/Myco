...
- **Homeostasis.** `immune` runs a 47-dimension lint against the seven hard rules. `senesce` winds down each session cleanly.
...
- **Homeostasis.** `immune` (47-dimension lint; `--fix` repairs mechanically where it can).
...
- **Downstream substrates.** `myco propagate` publishes. The data-driven per-host config writers live at `myco.boundary.install.clients` (v0.8.5 — the previous `myco.boundary.host_integration` adapter package was excreted as never-wired-into-production-code-paths).
