...
| **Cognitive** | 不只是存储。代谢（七步管道），验证（26 维免疫），自我建模（四层），进化规则本身可变。 | "它是否在代谢、验证、建模、进化？" |
...
基质不是无差别吞噬，而是选择性吞噬。write surface 白名单、pinned prefixes、digest 的 absorption gate、26 维 immune lint 的交叉约束，共同构成基质的身份边界。免疫系统不是辅助模块，它定义了 Myco 之所以是 Myco。
...
| 稳态 | immune 26 维 + hunger signals，自我调节维持稳定 |
...
| 4. 验证（免疫系统） | T 细胞 + 抗体 | immune 26 维 + hunger signals |
...
- src/myco/immune.py：26 维 Lint 免疫系统（五层体现第 4 层的实现）
