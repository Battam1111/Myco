...
## [2026-04-12] meta | gear2 discharge — 14 waves of inaction on `structural_bloat` is itself the recurring drift pattern. Wave 30→35 each saw the signal fire and each `deferred: primordia-compression` line either appeared with no reason (W13 violation in spirit) or was absorbed silently into "marathon discipline" — a degenerate use of W13 path b. The actual fix isn't more compression (the substrate is honestly bloated because everything is honestly load-bearing); it's recalibration of the threshold that operationalizes the discipline. This meta entry discharges gear2 and locks the lesson: friction signals that fire 6+ waves running without action are signals of *threshold drift*, not *operator laziness*. The Wave 26 D3 friction-driven ordering does NOT mean "always service the loudest signal"; it means "service the signal whose action repays the cost". For 6 waves the signal's prescribed action (compress) was foreclosed by a stronger doctrine (substrate immutability), so the right response was to recalibrate the signal — Wave 36 finally takes that step. Future drift to watch: if any signal fires for >5 waves without either action OR explicit recalibration craft, that's the same pathology re-emerging.

## [2026-04-12] milestone | **Wave 36 landed (kernel_contract, contract bump v0.27.0 → v0.28.0)** — `primordia_soft_limit` re-baseline 40 → 60. The first wave to take Wave 22 §B7 R2.4's pre-authorized "raise the soft limit" exit clause. Closes 6 consecutive waves of unaddressed `structural_bloat` hunger signal (firing since at least Wave 30). Wave 36 R0 audit found 47 active crafts in `docs/primordia/` are all load-bearing for current doctrine and impl modules; 2 SUPERSEDED-in-fact candidates (`metabolic_inlet_design_craft_2026-04-12.md` SUPERSEDED by W35 inlet_mvp_craft, `compression_primitive_craft_2026-04-12.md` SUPERSEDED by W30 compress_mvp_craft) have ~14 and ~16 reference depth respectively, with ~9 of those references in immutable historical surfaces (notes + log entries). Archive of either would force either widespread link rot or substrate immutability violation — both worse doctrinal trades than the bloat signal. Wave 22 §B7 R2.4 explicitly anticipated this exact case ("If creation velocity ever exceeds compression... e.g., wave 30+ where everything is still active, that's a real signal — probably time to raise the soft limit"). Wave 36 executes the re-baseline path. (1) `_canon.yaml` + `src/myco/templates/_canon.yaml` (mirrored): contract_version `v0.27.0 → v0.28.0`, synced_contract_version mirrored, `system.structural_limits.primordia_soft_limit` 40 → 60 with multi-line comment block citing Wave 22 §B7 R2.4 lineage + velocity calculation rationale. (2) `src/myco/notes.py::DEFAULT_STRUCTURAL_LIMITS["primordia_soft_limit"]` 40 → 60 (in-code default for fallback path). (3) `docs/contract_changelog.md` v0.28.0 entry: motivation (sensor without effector pathology), why-not-archive (reference depth + immutability), why-60-specifically (velocity calc: 47 + 14×1.57 ≈ 69 projected at W50, 60 = ~8 waves headroom = 1 forced re-eval per ~2 month period), full self-tests + hard-contracts + doctrine-coverage. (4) `docs/primordia/primordia_soft_limit_rebaseline_craft_2026-04-12.md` (NEW kernel_contract craft, status ACTIVE, target=current=0.90 floor for kernel_contract, rounds 3, full L13 schema). Round 1 per-option pressure validates 4 options A/B/C/D with explicit reference-depth measurements + Wave 22 quote-and-trace. Round 2 attacks A–F: A "re-baselining sweeps bloat under the rug" defended via Wave 22 §B7 R2.4's explicit distinction between sensor-without-effector pathology vs late-substrate calibration drift; B "60 is arbitrary" defended via velocity calculation showing 50/60/80 trade-offs; C "delete the limit entirely" defended via continuous compression open_problems §4 still being unsolved (signal is the only visibility into bloat trends); D "14 waves of inaction is the real bug" partially conceded — historical W13 violation (no `deferred:` reasons logged), this wave starts the healing; E "README staleness is the bigger bloat" conceded as honest L1 limitation, deferred to W37+; F "single threshold value isn't a contract" defended via existing convention (every kernel canon edit has been minor bump). Round 3 edge cases C1–C4 + decision lock D1–D9. (5) Verification: `myco lint` 19/19 PASS post-edit (L17 contract drift silent because synced_contract_version moves in lockstep with kernel; L15 craft reflex satisfied by new craft file in primordia/); `pytest tests/ -q` 22/22 PASS unchanged (this wave is pure config, no test changes); `myco hunger` post-edit shows `structural_bloat` signal SILENT (49 < 60), only `session_end_drift` gear2 remains and is discharged via the meta entry above this milestone. (6) Wave 36 evidence note `n_20260412T073525_f20e` (audit findings + reference depth measurements + alternatives considered) extracted via W32 `myco extract` (5th consecutive wave dogfooding W32 step verbs). Wave 36 decisions note `n_20260412T073625_269d` (D1–D9 + landing record + L1–L7 limitations + W37+ trajectory) integrated via W32 `myco integrate`. (7) D1–D9 captured: D1 primordia_soft_limit 40→60 in 3 places, D2 contract bump v0.27.0→v0.28.0 minor, D3 NO archive of any primordia file (Option A foreclosed by reference depth + immutability), D4 NO new lint dimension (sensor unchanged, only calibration moves), D5 NO split of primordia by phase (Option C: higher cost, no clear benefit), D6 NO new procedural rule (W13 still binding), D7 gear2 discharge in same wave via meta entry above this milestone, D8 README staleness honest acknowledgment as L1, D9 next forced re-evaluation expected ~Wave 44–46, do NOT pre-bind that wave's choice. (8) 7 honest limitations: L1 README staleness deferred (Attack E conceded), L2 continuous compression open_problems §4 only mitigated 8 waves not solved, L3 next re-baseline wave not pre-bound, L4 2 SUPERSEDED candidates remain SUPERSEDED-in-fact / ACTIVE-in-record (substrate immutability constraint), L5 60 may need adjustment if velocity changes, L6 other accumulated maintenance debt (W29b biomimetic renames, D-layer cross-ref, view audit log) not in scope per "one subsystem per wave", L7 no compression_overdue automatic-pressure mechanism. (9) Anchor coverage: anchor #4 (压缩即认知) unchanged — re-baseline is calibration not new mechanism. Wave 22 W13 procedural rule still active. Substrate immutability reinforced (re-baseline preserves all 30+ citations to the 2 SUPERSEDED candidates intact). (10) Marathon mode continues — Wave 37 begins next; per Wave 26 D3 (friction-driven ordering) NOT pre-scoped, will be decided by post-W36 friction. Wave 37+ candidates in priority order from Wave 36 craft §4.4: (a) README staleness sweep if reading-friction surfaces, (b) inlet_ripe / compression_overdue advisory signal generalized from W34 §3.3 #1, (c) cross-reference graph for inlet provenance from W34 §3.3 #2, (d) Wave 29b biomimetic file renames (long-deferred maintenance). Doctrine: re-baseline is honest only when archive is foreclosed AND the threshold drift is documented in a craft. The Wave 36 craft is the audit trail proving both conditions are met. Future re-baselines must follow the same template: per-option pressure validation, alternatives explicitly rejected, velocity calculation, future re-evaluation expected at <wave>. The discipline self-perpetuates via W13 + this craft as precedent.

## [2026-04-12] milestone | **Wave 37 landed (kernel_contract trigger surface touched, NO contract bump)** — README + doctrinal-surface staleness sweep. Closes Wave 36 §4.4 friction candidate (a), the highest-priority post-W36 reading-surface friction. Pure narrative-surface synchronization wave: zero behavior change, zero schema change, zero new lint dimension, zero canon mutation, zero test addition. Kernel_contract trigger surfaces ARE touched (mcp_server.py docstring + tool description + line 189 mode label, cli.py lint/immune parser help strings, immune.py docstring + L18 enumeration), so the Wave 12 craft reflex arc fired and was satisfied with a kernel_contract-class craft document BEFORE any edit landed — same precedent as Wave 35's design-then-impl split, applied to label sync. (1) `docs/primordia/readme_staleness_sweep_craft_2026-04-12.md` (NEW kernel_contract craft, status ACTIVE, target=current=0.90 floor for kernel_contract, rounds 3, full L13 schema, decision_class kernel_contract). Round 1 = 11-subsection per-surface inventory mapping every stale claim to its current-state replacement. Round 2 = 6 attacks A–F: A "this is just label maintenance, not worth a craft" defended via kernel_contract trigger surface discipline (mcp_server.py + cli.py help strings advertise lint dimensionality to MCP clients — silently lying = doctrine erosion); B "fix only the README and defer the rest" defended via root-cause-symmetry (the rot is everywhere because the SSoT was never enforced — partial fix = guaranteed re-rot); C "translate later, ship the English fix now" defended via mirror-divergence-asymmetry (3 mirrors compounded the rot independently across waves, deferring 中文/日本語 again would 3x the future surface area); D "drop the contract version footnotes entirely" partially conceded — body prose contract version inlines are inherently fragile (D4 of decisions), Wave 37 dropped multiple `(contract v0.8.0)` parentheticals to the simpler `(L0-L18)` form to avoid re-rotting on next bump; E "this should auto-detect not manual sweep" deferred to Wave 38+ candidate (LINT_DIMENSION_COUNT-from-ground-truth lint, see D7); F "kernel_contract for a label sync is overkill" defended via the reflex arc fires on trigger surface touches regardless of edit semantics — the right move is honor the reflex with a minimal-but-real craft, not bypass it. Round 3 = edge cases C1–C5 + D1–D8 decision lock + 15-step landing list. (2) Files edited (15 narrative surfaces + 1 craft + 1 evidence note + 1 decisions note): `README.md` (5 edits: badge `Lint-15%2F15 green` → `Lint-19%2F19 green`, pipeline tools list expanded to 8 verbs, Open Problems #6 rewritten to reflect Wave 18 seed shipped + W33 prune verb landed + remaining D-layer items, v0.x today/coming row "15-dimension" → "19-dimension" + Self-Model D seeded + Metabolic Inlet MVP scaffold W35 v0.27.0, verb table 7→15 rows with new evaluate/extract/integrate/compress/uncompress/prune/inlet/correct rows + lint marked `19-dimension (alias: myco immune)`); `README_zh.md` + `README_ja.md` (5 edits each, mirror); `MYCO.md` (~13 edits: 最后更新 → 2026-04-12, kernel contract `v0.9.0` → `v0.28.0` in 3 places including line 22 expanded status list, Phase ② chronicle expanded to include Waves 27–36, §任务 4 `📐 v2.0 ▱▱▱` → `🔄 ▰▰▱▱：Wave 34 设计 craft + Wave 35 MVP scaffold (myco inlet, v0.27.0) 落地`, Metabolic Inlet 性质 + 形状 sections rewritten with Wave 34/35 honest landing references + MVP edge notes, invariant clause `**18 维 lint (L0-L17)**` → `**19 维 lint (L0-L18)**`, provenance scaffold bullet added `inlet_origin / inlet_method / inlet_fetched_at / inlet_content_hash`, 不会做 → 已部分实现/不会做 honest 4-bullet supersession, indicator panel rationale `**18 维 L0-L17 全绿**` → `**19 维 L0-L18 全绿**`, §摘要 + phase bullets + doc index v1.x → v0.x post-rebase mapping); `src/myco/cli.py` (4 edits: lint + immune parser blocks both `18-dimension` → `19-dimension` in comments + help strings); `src/myco/immune.py` (4 edits: docstring `18-dimension` → `19-dimension`, `L0–L17` → `L0–L18`, added L18 compression integrity bullet, comment `# 18 immune dimensions` → `# 19`); `src/myco/mcp_server.py` (3 edits: line 9 module docstring tool list `Run 15-dimensional consistency checks (L0-L14)` → `Run 19-dimensional consistency checks (L0-L18)`, line 108 tool description `15-dimensional immune system` → `19-dimensional` with full L0-L18 enumeration replacing L0-L14, line 189 mode label `full (L0-L14)` → `full (L0-L18)`); `src/myco/migrate.py` line 169, `src/myco/init_cmd.py` line 115, `scripts/myco_init.py` line 104, `scripts/myco_migrate.py` line 217 (4 identical shim creation messages bumped 15→19); `CONTRIBUTING.md` line 140 (`15-dimension consistency checker (L0-L14, contract v0.8.0)` → `19-dimension consistency checker (L0-L18)` — dropped the parenthetical contract version per D4 to prevent re-rot); `wiki/README.md` line 35 (`应 15/15 绿` → `应 19/19 绿`); `docs/reusable_system_design.md` line 197 (`lint_dimensions: 15` → `19`, dropped `(contract v0.8.0)`); `docs/adapters/README.md` line 14 (`pre-release (v0.2.0, contract v0.8.0)` → `pre-release (package v0.2.0, kernel contract v0.28.0)` — labeled package vs kernel contract explicitly to disambiguate the dual versioning). (3) `docs/primordia/readme_staleness_sweep_craft_2026-04-12.md` (NEW, ~640 lines, kernel_contract class, 3 rounds, 0.90/0.90). (4) Decisions D1–D8 captured in note `n_20260412T081821_d4d0` (integrated via the W32 `myco integrate` verb — sixth consecutive wave dogfooding W32's verbs after W32 + W33 + W34 + W35 + W36): D1 single source of truth for LINT_DIMENSION_COUNT is `len(lint.py::checks)` and every other surface is a downstream cache that must be bumped in lockstep when a new dimension lands; D2 doctrinal-narrative surfaces (README, MYCO.md §, CONTRIBUTING) are NOT pinned by Hard Contract #2 (only log.md / contract_changelog.md / primordia/*.md / notes/*.md / examples/ascc/ are pinned), so they MUST be edited to track substrate state — treating them as effectively-pinned was the silent contributor to the rot; D3 translation mirrors must always land in the same wave/commit as the English source (README_zh.md and README_ja.md drifted independently across multiple prior waves, future README.md edits require simultaneous mirror edits); D4 contract version references in body prose are inherently fragile, keep them only at the SSoT (`_canon.yaml::system.contract_version`) — Wave 37 dropped multiple inline `(contract v0.8.0)` parentheticals to the simpler `(L0-L18)` form; D5 kernel_contract trigger surfaces (mcp_server.py + cli.py help) require a craft for ANY edit — even label-sync edits are subject to the Wave 12 craft reflex arc, the craft can be minimal but must exist BEFORE the edits land; D6 in-execution scope expansion is permitted when same-root-cause + same-risk-profile + same-file-already-being-edited (Wave 37 expanded mid-execution to cover MYCO.md contract version drift across body lines 4/22/136/153/154/211/212 + mcp_server.py line 189 missed in first sweep + docs/reusable_system_design.md + docs/adapters/README.md not in original §1 inventory but discovered during sweep); D7 Wave 38+ friction candidates from Wave 37 evidence ranked: (1) LINT_DIMENSION_COUNT-from-ground-truth lint (would have caught Wave 37's entire root cause at Wave 30 when L18 landed), (2) translation-mirror lint, (3) contract-version-inline lint — D7 ranking does NOT pre-bind Wave 38, it offers the candidates per Wave 26 D3 friction-driven ordering; D8 Wave 37 closes WITHOUT contract bump (label sync wave has no behavior/schema/Hard Contract change, kernel contract v0.28.0 stays). (5) Evidence note `n_20260412T080158_4f6b` (raw → digesting → extracted via the W32 `myco extract` verb after raw_backlog management) — captures the friction scan, ground-truth values verified, surfaces audited (12+), honest edges, verification surface. (6) Verification: `myco lint` 19/19 PASS pre-flight before commit (verified 08:16:35 + post-decisions-eat); `pytest tests/ -q` 22/22 PASS unchanged (Wave 37 adds zero tests by design); `myco hunger` regen post-landing pending (will run before commit). (7) Anchor coverage: NO anchor coverage shifts — Wave 37 changes labels not behavior; the substrate's actual capabilities are unchanged. The ANTI-shift is real however: the doctrine surfaces no longer LIE about substrate state, which restores honest self-description (anchor #1 substrate-vs-tool boundary integrity reinforced via no-longer-misrepresenting-the-checking-surface-to-MCP-clients). (8) **No contract bump**. v0.28.0 stays. Pure narrative-surface refresh, no canon mutation, no schema mutation, no Hard Contract block edit. (9) Marathon mode continues — Wave 38 begins next; per Wave 26 D3 (friction-driven ordering) NOT pre-scoped. Wave 38+ candidates from Wave 37 D7 in current ranking: (i) LINT_DIMENSION_COUNT-from-ground-truth lint (highest leverage — would have caught the Wave 37 root cause prophylactically), (ii) translation-mirror lint, (iii) contract-version-inline lint, (iv) inlet_ripe / compression_overdue advisory signal (W34 §3.3 #1, deferred from W37), (v) cross-reference graph for inlet provenance (W34 §3.3 #2), (vi) Wave 29b biomimetic file renames (long-deferred maintenance). Final selection by next-wave opening assessment per W26 D3. Doctrine: a substrate that lies about its own checking surface is a substrate that has stopped reading itself. The Wave 12 craft reflex arc fired correctly when mcp_server.py was touched — the reflex is for ANY trigger surface mutation, not just behavioral ones — and Wave 37 honored it with a real craft document. The Wave 22 W13 deferred-with-reason discipline applies: this wave's `deferred:` line is empty because Wave 37 closed every in-scope surface and explicitly logged D7 candidates to the next wave's friction-evaluation step. Honest self-description is the cheapest reflex to maintain and the most expensive to neglect — every wave that lets it slip pays compounding interest. deferred: primordia-compression (W22 W13 check: primordia count now 50 ≥ soft limit 60 — Wave 37 added 1 more primordia file `readme_staleness_sweep_craft_2026-04-12.md`, structural_bloat signal SILENT post-Wave-36 re-baseline — 50 < 60). advisory: session_end_drift gear2 (24 entries since last meta reflection, threshold 15) — acknowledged as LOW-severity advisory; meta reflection deferred per marathon discipline.

## [2026-04-12] milestone | **Wave 38 landed (kernel_contract, contract bump v0.28.0 → v0.29.0)** — L19 lint dimension count consistency. Closes Wave 37 D7 followup #1, the highest-ranked candidate from Wave 37's manual sweep. (1) NEW lint dimension `lint_dimension_count_consistency` in `src/myco/lint.py` (~120 lines): scans 4 HIGH-severity narrative surfaces (README.md, README_zh.md, README_ja.md, MYCO.md) + 11 MEDIUM-severity surfaces (CONTRIBUTING.md, wiki/README.md, src/myco/{cli,immune,mcp_server,migrate,init_cmd}.py, scripts/myco_{init,migrate}.py, docs/reusable_system_design.md, docs/adapters/README.md) for any of 5 regex patterns whose integer drifts from `len(FULL_CHECKS)`. Patterns: README badge `Lint-(\d+)%2F(\d+)`, English `(\d+)[- ]dimension(?:al)?`, CJK `(\d+)\s*(?:维|次元)\s*(?:lint|L0|の|的)?` (covers 中文 and 日本語), L range `L0[-–](?:L)?(\d+)` with quick-mode subset exemption, and pass ratio `(\d+)/(\d+)\s+(?:green|绿|PASS|pass)`. Wave 37 D1 SSoT principle now structurally enforced — every cache MUST mirror `len(FULL_CHECKS)` or trip L19 at the next lint run. (2) Module-level FULL_CHECKS / QUICK_CHECKS tuples in `src/myco/lint.py` (Wave 38 D2): refactored `main()`'s local `checks = [...]` block into immutable module-level tuples that L19 (and any future consumer) can reference as the SSoT. Tuples prevent accidental mutation; `main()` casts to list for its existing local-mutable usage pattern. (3) `src/myco/mcp_server.py` dispatch refactor (Wave 38 D9 — scope expansion): the pre-existing `myco_lint` MCP tool was discovered to maintain its OWN local `checks = [...]` list that had drifted to L0-L14 only and was silently dropping L15-L18 from MCP-mode lint reports. MCP users were running 15-dim lints while CLI users were running 19-dim — a real behavioral bug of exactly the SSoT-violation class L19 was designed to prevent at the narrative layer. Refactored to import `FULL_CHECKS, QUICK_CHECKS` from `myco.lint` directly and dispatch through them, eliminating the silent dispatch drift. Module docstring + tool annotation title + tool description (with full L0-L19 enumeration adding L19) + mode label all bumped. (4) Pinned-surface allowlist (Wave 38 D5): L19 explicitly does NOT scan log.md, contract_changelog.md, docs/primordia/*, notes/*, examples/ascc/*, or src/myco/lint.py itself. The first five are append-only/pinned per Hard Contract #2 — historical claims must be preserved verbatim including their period-correct dimension counts. lint.py is excluded to avoid the own-file self-reference loophole. (5) 4 unit tests in `tests/unit/test_lint_dimension_count.py` (Wave 38 D8): test_l19_clean_substrate_passes (base case → 0 issues), test_l19_badge_drift_caught_high (HIGH severity), test_l19_multi_pattern_co_match (≥3 issues per drifted line, no dedup), test_l19_severity_split_high_vs_medium (HIGH for README, MEDIUM for CONTRIBUTING). Each test pulls EXPECTED from `len(FULL_CHECKS)` at import time so assertions also derive from the SSoT and never need hardcoded updates. (6) 15 narrative surface bumps to '20-dimension / L0-L19 / 20/20' in lockstep — README badges (en/zh/ja) `Lint-19%2F19` → `Lint-20%2F20`, MYCO.md §指标面板 lint health value 0.68 → 0.70 + L19 mention, immune.py docstring + dimension list bullet add L19, CONTRIBUTING.md, the migrate/init scripts, docs/reusable_system_design.md, docs/adapters/README.md. L19 caught two surfaces even Wave 37's manual sweep MISSED at first run: README_zh.md:246 (`19 维基质健康检查` — Wave 37 bumped the doc but missed this verb table row) and MYCO.md:226 (`14 维度自动化一致性检查` — older drift, predates L18, has been rotting since at least Wave 24). This is L19's first dogfood proof: the lint detected real, in-substrate drift on its very first run that the human reviewer had missed across two manual sweeps. (7) Contract bump v0.28.0 → v0.29.0 in `_canon.yaml` + `_canon.yaml::synced_contract_version` (kernel self-reference) + `src/myco/templates/_canon.yaml::synced_contract_version` mirror. v0.29.0 changelog entry (~110 lines) covering motivation, 8 numbered changes, verification, backward-compatibility, and forward path appended to `docs/contract_changelog.md`. (8) Verification: `myco lint` 20/20 PASS (after fixing residual L19 hits + L16 boot brief refresh via `myco hunger`); `pytest tests/ -q` 26/26 PASS (22 prior + 4 Wave 38); `myco hunger` healthy with no `dimension_count_drift` channel firing. Note: n_20260412T084315_7eb1 (integrated). Craft: `docs/primordia/lint_dimension_count_consistency_craft_2026-04-12.md` (kernel_contract, 3 rounds, target=current=0.90, single-author equality per craft_protocol §4 floor). Doctrine: every assertion of system shape that does not derive from a structural SSoT will silently rot — the only sustainable anti-rot is to make the SSoT mechanically reachable from every cache that mirrors it, then have an automated check enforce the mirror. Wave 30 to Wave 37 was 7 waves of invisible drift; L19 makes that class structurally impossible from now on.

## [2026-04-12] milestone | **Wave 39 landed (kernel_contract, contract bump v0.29.0 → v0.30.0)** — L20 translation mirror consistency. Closes Wave 37 D7 followup #2 (the second-highest-leverage Wave 39 candidate per Wave 26 D3 friction-driven ordering). (1) NEW lint dimension `lint_translation_mirror_consistency` in `src/myco/lint.py` (~80 lines) plus the 75-line `_count_skeleton(content)` helper. The 5-tuple skeleton `(h2_count, h3_count, code_count, table_rows, badge_count)` is the parity unit (Wave 39 D1) — if all 3 locale READMEs (`README.md`/`README_zh.md`/`README_ja.md`) share the same tuple, L20 returns empty; otherwise L20 emits one HIGH issue per drifted component naming the file and the component (h2/h3/code/table_rows/badge). The parser is code-fence-aware (Wave 39 D6): H2/H3 lines inside ```` ``` ```` blocks are NOT counted, guarding against false positives from inline markdown samples. The skip-marker `<!-- l20-skip -->` (Wave 39 D5) on the previous non-blank line excludes the immediately following H2/H3 from counts — escape hatch for genuinely locale-specific sections like a Chinese-only "致谢" or Japanese-only "謝辞" without forcing operators to remove locale-native content. Reference is `README.md` (Wave 39 D2); fallback if missing picks the locale README with the highest h2_count (greenfield projects). Fewer than 2 locale READMEs → silent pass (nothing to mirror against). FULL_CHECKS auto-extends to 21 (Wave 38 D2 SSoT discipline), so L19 immediately dogfooded the transition: every narrative surface still claiming "20-dimension" surfaced as a HIGH issue at the first Wave 39 lint run, forcing the entire change-set to land in lockstep — proving Wave 38's anti-rot mechanism works through a real cross-wave transition. (2) 15+ narrative surfaces bumped 20-dimension/L0-L19/Lint-20%2F20 → 21-dimension/L0-L20/Lint-21%2F21: README.md (badge + version row + L20 bullet), README_zh.md (badge + 今天 row + 21 维 verb table count), README_ja.md (badge + 今日 row + L20 bullet + 21 次元 verb table count), MYCO.md (4 lines: 21 维 lint headline, lint_coverage_confidence row, contract version row, 21 维度 verb table), CONTRIBUTING.md (lint module comment), wiki/README.md (`myco lint` 应 21/21 绿), docs/reusable_system_design.md (lint_dimensions: 21), src/myco/cli.py (replace_all 20-dimension → 21-dimension + Wave 29 immune comment), src/myco/init_cmd.py (lint shim message), scripts/myco_init.py (mirror), src/myco/migrate.py (shim message), scripts/myco_migrate.py (mirror), src/myco/immune.py (docstring 20→21 + L0-L19→L0-L20 + L20 bullet + add lint_dimension_count_consistency + lint_translation_mirror_consistency to imports + __all__ — closes a Wave 38 oversight where L19 was missing from immune.py exports), src/myco/mcp_server.py (4 edits: tools header, tool annotation title, full L0-L20 enumeration in docstring, mode label `full (L0-L19)` → `full (L0-L20)`), src/myco/lint.py (header `20-Dimension` → `21-Dimension`, dimension table row added, FULL_CHECKS comment `L0-L19` → `L0-L20`). Bonus catch: the Wave 38 oversight in immune.py (missing L19 export) was discovered and fixed in the same patch. (3) 4 unit tests added at `tests/unit/test_lint_translation_mirror.py` (Wave 39 D8): `test_l20_clean_substrate_passes` (D1 base case — 3 parallel locale READMEs produce zero issues), `test_l20_section_drop_caught` (D1+D4 principal scar — README_zh.md missing one `##` surfaces as HIGH naming the h2 component), `test_l20_skip_marker_respected` (D5 escape hatch — marked section excluded from counts via both end-to-end lint and direct `_count_skeleton` assertion), `test_l20_code_fence_aware` (D6 parser invariant — `## fake heading` inside a `\`\`\`markdown` fence does not count; parallel files compute identical skeletons). Suite count advances 26 → 30. Each test pulls `_count_skeleton` and `lint_translation_mirror_consistency` directly (no CLI roundtrip) and uses the existing `_isolate_myco_project` fixture from `tests/conftest.py`. (4) Contract bump v0.29.0 → v0.30.0 in `_canon.yaml` + `src/myco/templates/_canon.yaml` (synced_contract_version mirror keeps L17 quiescent) + `docs/contract_changelog.md` (~150-line v0.30.0 entry with 9 numbered changes mirroring Wave 39 D1-D9, motivation, verification, backward-compat note, forward path, 4 honest limitations L1-L4). Minor bump matches the precedent set by L18 (v0.26.0), L19 (v0.29.0), and Wave 24 L17. (5) Authoritative craft `docs/primordia/translation_mirror_lint_craft_2026-04-12.md` (kernel_contract class, 3 rounds, current_confidence = target_confidence = 0.90, ~380 lines) covers §0 problem definition, §1 round 1 design (D1-D6 first-pass), §2 round 2 attacks A-F (multi-locale fork tax, granularity-too-coarse, escape hatch abuse, parser brittleness, dogfood against own substrate, false-positive rate), §3 round 3 edge cases C1-C7 + D1-D9 final shape, §4 conclusion with D1-D9 + 15-item landing list + L1-L4 limitations + Wave 40+ disposition (#3 contract-version-inline lint). Decisions note `n_20260412T090405_8f76` integrated from raw → integrated. Closing doctrine: **Wave 38 made the dimension count a single source of truth and gave it ground-truth lint coverage; Wave 39 makes the locale READMEs mirror each other and gives that mirror ground-truth lint coverage. Together the two waves close the two highest-leverage silent-rot surfaces in the substrate's user-facing first-impression layer. Future waves can add new dimensions, new sections, new badges — and all three locales will move in lockstep because the immune system now blocks any single-locale drift at the lint boundary.** Final state: `myco lint` 21/21 PASS (post-hunger refresh), `pytest tests/ -q` 30/30 PASS, `myco hunger` 94 notes / 0 promote_candidates / healthy. Wave 40+ continues Wave 26 D3 friction-driven ordering with the third Wave 37 D7 followup — the contract-version-inline lint dimension that catches `__version__` strings + inline `v0.x.y` doc references drifting from `_canon.yaml::system.contract_version`.
