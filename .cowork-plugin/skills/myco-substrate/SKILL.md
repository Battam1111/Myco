...
name: myco-substrate
description: >
  Work with a Myco cognitive substrate — any project directory with `_canon.yaml`
  and `MYCO.md` at its root. Trigger when the workspace contains `_canon.yaml`
  (it IS a Myco substrate); the user mentions "substrate", "Myco", or any of
  the 18 fungal verbs (germinate / hunger / eat / sense / forage / assimilate
  / digest / sporulate / traverse / propagate / immune / senesce / fruit /
  molt / winnow / ramify / graft / brief); a prior tool response's
  `substrate_pulse` referenced Myco; the user asks about agent memory, long-
  term context, cross-session state, or agent-first cognitive architecture in
  the context of a project that has `_canon.yaml`. Myco is NOT a memory tool
  and NOT a RAG framework — skip this skill when the user wants vector
  retrieval, LangChain-style pipelines, or ephemeral chat-scoped memory.
...
## The 18 verbs, grouped by subsystem
...
- **Circulation** — `myco_traverse` (walk the mycelium graph, report
  anastomotic health), `myco_propagate` (publish integrated/distilled
...
## Seven hard contract rules (R1–R7)

Every session is governed by:

- **R1 Boot ritual** — call `myco_hunger` as the first substantive action
  of every session.
- **R2 Session-end** — call `myco_senesce` before compaction / at session
...
- `contract_version` — the R1–R7 contract version in force.
...
- **永恒进化 (Eternal Evolution)** — canon, lint dimensions, verbs, even
  the R1-R7 contract itself are mutable through the governed
...
- **Do not** write files outside the workspace's
  `_canon.yaml::system.write_surface.allowed` — that's R6.
...
- `L1_CONTRACT/protocol.md` — R1-R7 full text.
- `L2_DOCTRINE/` — one doctrine page per subsystem.
- `src/myco/surface/manifest.yaml` — the single source of truth for
  the 18 verbs' arg shapes. Both CLI and MCP tool surface derive from
  it.
