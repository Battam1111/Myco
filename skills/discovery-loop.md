...
2. **For each candidate** (top 3 by priority):
   ...
   c. **Ingest**: If passes evaluation, call `myco_inlet` with the content + provenance
   d. **Log**: Record the decision in `myco_log` (what was found, why ingested/rejected)
3. **Verify**: Call `myco_hunger` to confirm signals improved.

## Constraints

- Max 3 ingestions per session (prevent bloat)
- Always check `myco_search` first to avoid duplicates
