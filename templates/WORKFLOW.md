...
4. 防御/修正
5. 迭代至所有攻击点有可信防御
6. 收尾：① 更新任务队列 ② 完整记录 → docs/current/ ③ 结论萃取 → wiki/CLAUDE.md ④ log.md
...
- **Context Priming**（CLAUDE.md 热区 ≤5 行）：工具手感描述
...
| 步骤 | 操作 | 验证 |
|------|------|------|
| ① 索引 | CLAUDE.md Wiki 索引表添加新行 | grep 页面名 |
...
**流程**：评估价值 → 清理硬编码+注释 → 复制到 scripts/ → 更新 CLAUDE.md §4
...
标注位置：CLAUDE.md §3 文档索引表。`[COMPILED]`/`[SUPERSEDED]` 文档通常不需要读取。
