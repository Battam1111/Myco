...
- **Claude Desktop / Cowork** — `myco-install host cowork` が MCP エントリと `myco-substrate` オンボーディングスキルを各 Cowork ワークスペースに一括展開。次回セッションからエージェントが認識します。
...
- **Cowork（Claude Desktop local-agent-mode）。** `myco-install host cowork` で MCP + `myco-substrate` スキルが一括設置され、次セッションでエージェントが `_canon.yaml` を見た瞬間に R1-R7 に従います。Cowork は hooks を公開しないため、boot ritual は skill に載せています — 背景は [`INSTALL.md`](docs/INSTALL.md) と `.cowork-plugin/README.md` を参照。
