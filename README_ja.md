...
**Cowork ユーザー向け**：`myco seed` はプロジェクトレベルの設定を生成しますが、Cowork はプロジェクトの `.mcp.json` を読み込みません。Cowork で完全な MCP 自動注入を得るには、`myco.plugin` をインストールしてください。MCP サーバー、スキル、セッションフックを一つのファイルにまとめています。詳細は `docs/agent_protocol.md` §0.5 を参照。
...
- 🤖 **あなたはただ話すだけ**。25のツールが全自動。人間は技術的な詳細を一切知らなくていい。
