...
> 最后更新：2026-04-12
> **知识系统**：[Myco](https://github.com/Battam1111/Myco) v0.2.0（框架自用，pre-release；kernel contract v0.31.0）
...
> 🫀 **Boot signals** (last `myco hunger`: `2026-04-12T01:33:09Z`)
>
...
- **不变量**：所有摄取过的内容必须通过现有 **22 维 lint (L0-L21)**；任何降低 lint pass 率的摄取必须被拒绝
...
| `lint_coverage_confidence` | 0.74 | **22 维 L0-L21 全绿** + Wave 25 tests/ 基础设施 + Wave 38 L19 维度计数自洽 lint + Wave 39 L20 翻译镜像 lint + Wave 40 L21 contract version inline lint（v0.31.0：substrate 现在结构性防护内联 contract 版本号漂移，forward-looking pattern + 8 项历史标记过滤，操作员 skip-marker 仍是逃生舱），双路径（myco.lint + scripts/lint_knowledge.py shim）一致；bootstrap ceiling 上调至 0.74（Wave 40 closure：L21 闭合 Wave 37 D7 followup triple，三连补丁封堵 contract 漂移最后一类反例） | Waves 18/20/21/23/24 craft records + Wave 25 tests landing + Wave 38 L19 + Wave 39 L20 + Wave 40 L21 crafts |
...
Myco 是一个可自进化的 AI 延伸认知基质框架。它给 AI agent 提供持久记忆、结构化知识和跨会话的自进化能力。v0.2.0 已可从源码安装（PyPI 公开发布准备中），提供 `myco init / migrate / lint / immune / eat / evaluate / extract / integrate / digest / view / compress / uncompress / prune / inlet / hunger / forage / upstream / correct / molt` CLI。当前 kernel contract v0.31.0（含 Upstream Protocol v1.0 + Craft Protocol v1 + 22 维 lint L0-L21 + Self-Model D 层种子 + Metabolic Inlet MVP scaffold + Wave 38 L19 维度计数自洽 lint + Wave 39 L20 翻译镜像 lint + Wave 40 L21 contract version inline lint）。
...
| 脚本 | 用途 |
|------|------|
| `scripts/lint_knowledge.py` | 22 维度自动化一致性检查（对照 `_canon.yaml`） |
