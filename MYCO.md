...
> 🫀 **Boot signals** (last `myco hunger`: `2026-04-14T13:51:36Z`)
>
...
**当前阶段**：v0.3.0 beta — PyPI 已发布，Show HN 待执行（尚未 1.0）；Post-Metamorphosis — Agent-First 有机体已建成。**Metamorphosis plan 完成**，kernel contract v0.45.0。关键里程碑：29 维 lint (L0-L28) + 25 MCP tools + 443 tests + Signal-to-Action hunger 自动执行闭环 + Link Graph 菌丝互联 + Semantic Cohort Intelligence + Inlet Trigger Policy + Continuous Compression + Session Memory FTS5 + **Wave 54 Agent-First Integration（hunger execute=true + scheduled metabolic cycle）**
...
| 指标 | 值 | rationale | 证据锚 |
|------|----|-----------|--------|
| `v1_launch_progress` | 0.65 | PyPI v0.3.0 已发布；kernel contract v0.45.0；26 lint + Agent Review + 25 MCP tools + 443 tests；dist/ 1.1.0 wheel 已清理。剩余：README v1 里程碑对齐、Show HN launch | PyPI 0.3.0 live, Metamorphosis plan |
...
| `lint_coverage_confidence` | 0.85 | **29 维 L0-L28 全绿** + 443 tests（from 38）+ 29 modules + Waves A1-E3 新增 evolve.py/redact.py/project.py 全部测试 + hybrid lint（mechanical 28 dims + Agent Review 4 items）+ inline frontmatter validation on writes + hunger sidecar push-mode。上调至 0.85 | Metamorphosis plan commits + 183/183 pytest + 29/29 lint + Agent Review |
...
Myco 是一个 Agent-First 共生认知有机体框架。它给 AI agent 提供持久记忆、结构化知识和跨会话的自进化能力。v0.3.0 已发布至 PyPI（推荐：`git clone` + `pip install -e ".[mcp]"`；快速预览：`pip install myco`），提供 `myco seed / graft / immune / genome / assimilate / eat / correct / digest / evaluate / extract / integrate / observe / condense / expand / prune / absorb / hunger / forage / mycelium / colony / memory / diagnose / propagate / rename` CLI。当前 kernel contract v0.45.0（26 维 lint + Agent Review hybrid 免疫 + 25 MCP tools + 443 tests + hunger auto-execute + `myco seed --agent claude` 一键就位 + evolve.py 自我进化 + predict_knowledge_needs 需求预判）。
