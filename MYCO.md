...
> 最后更新：2026-04-12
> **知识系统**：[Myco](https://github.com/Battam1111/Myco) v0.2.0（框架自用，pre-release；kernel contract v0.45.0）
...
> 🫀 **Boot signals** (last `myco hunger`: `2026-04-12T13:29:01Z`)
>
> - [REFLEX HIGH] raw_backlog: 27 pure-raw notes (digest_count=0, non-exempt source) exceed threshold 10. IMMUTABLE REFLEX (W1 autopilot, con...
>
...
| `compression_discipline_maturity` | 0.55 | W48 cohort suggest + W50 compression_pressure + `--cohort auto` + metabolic-cycle skill 含 discovery loop + auto-evolve step + W54 hunger auto-execute + Wave C1-C3 skill evolution with constraint gates。但仍无大批量真实压缩运行（仅 5 次 excretion），上调 0.50→0.55 | Metamorphosis Phase C+D+E + 122 tests |
...
Myco 是一个可自进化的 AI 延伸认知基质框架。它给 AI agent 提供持久记忆、结构化知识和跨会话的自进化能力。v0.2.0 已可从源码安装（PyPI 公开发布准备中），提供 `myco init / migrate / lint / eat / evaluate / extract / integrate / digest / view / compress / uncompress / prune / inlet / hunger / forage / upstream / correct / graph / cohort / session` CLI。当前 kernel contract v0.45.0（23 维 lint + Agent Review hybrid 免疫 + 19 MCP tools + 122 tests + hunger auto-execute + `myco init --agent claude` 一键就位 + evolve.py 自我进化 + discover.py 主动觅食 + predict_knowledge_needs 需求预判）。
...
| 文档 | 状态 | 内容 |
|------|------|------|
| `docs/primordia/archive/positioning_craft_2026-04-09.md` | [ACTIVE] | **深度定位辩论（6 轮，92% 置信度）**：OpenClaw 威胁分析、tagline→metabolism、Gear 4 已验证 |
...
| `docs/primordia/archive/examples_design_craft_2026-04-09.md` | [ACTIVE] | examples/ 设计辩论：Gear 4 生命周期展示定位 + 社区管线，置信度 84% |
