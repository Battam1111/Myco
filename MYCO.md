...
> 🫀 **Boot signals** (last `myco hunger`: `2026-04-12T00:20:54Z`)
>
...
- **不变量**：所有摄取过的内容必须通过现有 **19 维 lint (L0-L18)**；任何降低 lint pass 率的摄取必须被拒绝
...
| `lint_coverage_confidence` | 0.68 | **19 维 L0-L18 全绿** + Wave 25 tests/ 基础设施落地（v0.24.0：`tests/conftest.py::_isolate_myco_project` autouse + `tests/unit/test_notes.py` 4 seed tests），双路径（myco.lint + scripts/lint_knowledge.py shim）一致；bootstrap ceiling 限制 0.70 以内（Wave 26 re-audit：值暂不上调，待 Wave 27+ 友商数据） | Waves 18/20/21/23/24 craft records + Wave 25 tests landing |
...
Myco 是一个可自进化的 AI 延伸认知基质框架。它给 AI agent 提供持久记忆、结构化知识和跨会话的自进化能力。v0.2.0 已可从源码安装（PyPI 公开发布准备中），提供 `myco init / migrate / lint / immune / eat / evaluate / extract / integrate / digest / view / compress / uncompress / prune / inlet / hunger / forage / upstream / correct / molt` CLI。当前 kernel contract v0.28.0（含 Upstream Protocol v1.0 + Craft Protocol v1 + 19 维 lint L0-L18 + Self-Model D 层种子 + Metabolic Inlet MVP scaffold）。
