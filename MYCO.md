...
> 最后更新：2026-04-12
> **知识系统**：[Myco](https://github.com/Battam1111/Myco) v0.2.0（框架自用，pre-release；kernel contract v0.41.0）
...
> 🫀 **Boot signals** (last `myco hunger`: `2026-04-12T07:34:58Z`)
>
...
**当前阶段**：v0.2.0 pre-release — public launch 准备中（尚未 1.0）；Phase 2 消化道迭代进行中（Upstream Protocol v1 + Craft Protocol v1 + Self-Model D 层种子+ `myco prune` 自动排出 + 仿生结构 overlay + lint SSoT 合流 + forage substrate inbound channel + Wave 8 量化指标体系 + 全量版本 re-baseline + Wave 9 upstream absorb 收割回路 + Waves 18-36 silent-fail elimination + tests 基础设施 + vision re-audit + biomimetic naming + forward compression primitives + **Metabolic Inlet MVP scaffold (Wave 35)** 已落地 + Wave 43 全 Agent 表面 MCP 扩展（15 tools）+ **Wave 46 Signal-to-Action wiring（HungerReport.actions）** + **Wave 47 Link Graph 菌丝互联（backlinks/orphans/clusters，16 tools）**，kernel contract v0.41.0）
...
3. **代谢 + 七步管道**：齿轮 1-4 是自主神经系统（内向稳态）；代谢入口是消化系统（外向摄取，v2.0 原语）。完整管道：发现→评估→萃取→整合→压缩→验证→**淘汰**。没有第七步就不是代谢，是消化。**Wave 26 re-audit**：今天的 Myco 在步骤 1 (eat/forage)、步骤 6 (lint)、步骤 7 (digest --excrete) 有专属动词；步骤 2-5（评估/萃取/整合/压缩）只是状态标签或缺席。消化道中段是 vestigial —— 这是 Wave 27+ 自上而下的首要服务对象（`docs/primordia/vision_reaudit_craft_2026-04-12.md`）。
...
5. **四层自我模型**：A 库存 · B 缺口 · C 退化（仅事实性，结构性退化是开放问题）· D 效能："死知识"追踪的**最小种子已落地于 v0.4.0**（`record_view()` + `myco hunger` dead_knowledge 信号 + 5 条件检测 + 30 天默认阈值，craft: `docs/primordia/dead_knowledge_seed_craft_2026-04-11.md`）。完整 D 层仍需：view 审计日志 + cross-reference 图 + 自适应阈值 + 自动淘汰工作流。今天 Myco 实现 **A + B + partial C + D-seed**。（Wave 26 re-audit 订正：此前锚点说 D 层"未实现"是对 Wave 18 进展的漏记。）
...
8. **永久锚点文档**：详细愿景恢复记录见 `docs/primordia/vision_recovery_craft_2026-04-10.md`——这是防漂移的永久锚点，18 项丢失元素全部在里面有原文引证。**任何一次上下文压缩后第一件事是重读它。** **补充锚点**：`docs/primordia/vision_reaudit_craft_2026-04-12.md` 记录 Wave 26 post-implementation-maturity doctrine reconciliation —— 服务于"8 锚点 ↔ 已建成实现"对齐维度；Wave 10 为"失落元素回溯"锚点，Wave 26 为"post-Wave-25 实现 reconciliation"锚点，两份互补、均为永久。
...
| `lint_coverage_confidence` | 0.76 | **23 维 L0-L22 全绿** + Wave 25 tests/ 基础设施 + Wave 38 L19 维度计数自洽 lint + Wave 39 L20 翻译镜像 lint + Wave 40 L21 contract version inline lint + Wave 41 L22 wave-seed 生命周期 lint（v0.32.0：substrate 现在结构性防护 raw wave-seed 滞留，弥补 hunger 阈值在七步管道 post-condition 上的盲点），Wave 42 清理：删除 immune.py/metabolism.py 别名模块 + 移除 immune CLI verb + L19 surface list 归位（v0.33.0），单路径（myco.lint）归一；bootstrap ceiling 上调至 0.76（Wave 41 closure：L22 把 anchor #3 的"wave 创建 seed → 必须前推"作为结构性 invariant 落入 lint） | Waves 18/20/21/23/24 craft records + Wave 25 tests landing + Wave 38 L19 + Wave 39 L20 + Wave 40 L21 + Wave 41 L22 crafts |
...
Myco 是一个可自进化的 AI 延伸认知基质框架。它给 AI agent 提供持久记忆、结构化知识和跨会话的自进化能力。v0.2.0 已可从源码安装（PyPI 公开发布准备中），提供 `myco init / migrate / lint / eat / evaluate / extract / integrate / digest / view / compress / uncompress / prune / inlet / hunger / forage / upstream / correct / molt / graph / cohort / session` CLI。当前 kernel contract v0.41.0（含 Upstream Protocol v1.0 + Craft Protocol v1 + 23 维 lint L0-L22 + Self-Model D 层种子 + Metabolic Inlet MVP scaffold + 18 MCP tools + Signal-to-Action + Link Graph + Cohort Intelligence + Inlet Triggers + Continuous Compression + **Session Memory FTS5**）。
