...
> 🫀 **Boot signals** (last `myco hunger`: `2026-04-12T00:45:07Z`)
>
...
- **不变量**：所有摄取过的内容必须通过现有 **20 维 lint (L0-L19)**；任何降低 lint pass 率的摄取必须被拒绝
...
| `lint_coverage_confidence` | 0.70 | **20 维 L0-L19 全绿** + Wave 25 tests/ 基础设施 + Wave 38 L19 维度计数自洽 lint（v0.29.0：substrate 现在结构性防护自身的标签飘移，`len(FULL_CHECKS)` 即权威），双路径（myco.lint + scripts/lint_knowledge.py shim）一致；bootstrap ceiling 上调至 0.70（Wave 38 closure：L19 是结构性升级，证据足够将 confidence 从 0.68→0.70） | Waves 18/20/21/23/24 craft records + Wave 25 tests landing + Wave 38 L19 craft |
...
Myco 是一个可自进化的 AI 延伸认知基质框架。它给 AI agent 提供持久记忆、结构化知识和跨会话的自进化能力。v0.2.0 已可从源码安装（PyPI 公开发布准备中），提供 `myco init / migrate / lint / immune / eat / evaluate / extract / integrate / digest / view / compress / uncompress / prune / inlet / hunger / forage / upstream / correct / molt` CLI。当前 kernel contract v0.29.0（含 Upstream Protocol v1.0 + Craft Protocol v1 + 20 维 lint L0-L19 + Self-Model D 层种子 + Metabolic Inlet MVP scaffold + Wave 38 L19 维度计数自洽 lint）。
...
| 脚本 | 用途 |
|------|------|
| `scripts/lint_knowledge.py` | 20 维度自动化一致性检查（对照 `_canon.yaml`） |
