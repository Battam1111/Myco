...
> 🫀 **Boot signals** (last `myco hunger`: `2026-04-14T02:51:26Z`)
>
...
**当前阶段**：v0.3.0 beta — PyPI 已发布，Show HN 待执行（尚未 1.0）；Post-Metamorphosis — Agent-First 有机体已建成。**Metamorphosis plan 完成**，kernel contract v0.45.0。关键里程碑：25 维 lint (L0-L24) + 19 MCP tools + 282 tests + Signal-to-Action hunger 自动执行闭环 + Link Graph 菌丝互联 + Semantic Cohort Intelligence + Inlet Trigger Policy + Continuous Compression + Session Memory FTS5 + **Wave 54 Agent-First Integration（hunger execute=true + scheduled metabolic cycle）**
...
6. **内化的变异-选择**：变异和选择都发生在 Agent+Organism 内部。Agent 产生变异（eat/digest/condense/absorb），Organism 的免疫系统执行选择（lint 25 维 + hunger signals + compression_pressure + dead_knowledge detection）。人类提供方向和信任，不需要做技术选择。透明性不可变——不是为了让人类选择，而是为了可审计性；没有审计能力 → 失去信任 → 系统被废弃。**永恒进化**（Perpetual Evolution）是三条不可变律之一：**停滞即死**——代谢循环停止运转的有机体会退化为静态缓存。
...
| 指标 | 值 | rationale | 证据锚 |
|------|----|-----------|--------|
| `v1_launch_progress` | 0.65 | PyPI v0.3.0 已发布；kernel contract v0.45.0；25 lint + Agent Review + 19 MCP tools + 282 tests；dist/ 1.1.0 wheel 已清理。剩余：README v1 里程碑对齐、Show HN launch | PyPI 0.3.0 live, Metamorphosis plan |
...
| `lint_coverage_confidence` | 0.85 | **25 维 L0-L24 全绿** + 282 tests（from 38）+ 29 modules + Waves A1-E3 新增 evolve.py/redact.py/project.py 全部测试 + hybrid lint（mechanical 24 dims + Agent Review 4 items）+ inline frontmatter validation on writes + hunger sidecar push-mode。上调至 0.85 | Metamorphosis plan commits + 183/183 pytest + 25/25 lint + Agent Review |
...
| 脚本 | 用途 |
|------|------|
| `scripts/lint_knowledge.py` | 25 维度自动化一致性检查（对照 `_canon.yaml`）；推荐通过 `myco immune` 调用 |
