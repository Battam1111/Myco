...
> 最后更新：2026-04-13
> **知识系统**：[Myco](https://github.com/Battam1111/Myco) v0.3.1（[PyPI 已发布](https://pypi.org/project/myco/0.3.1/)；kernel contract v0.46.0）
...
**Boot ritual**: Call `myco_hunger(execute=true)` as your FIRST action every session. Read the signals below — if anything other than "healthy", address it before doing the human's task.

<!-- MYCO-BOOT-SIGNALS:BEGIN (auto-generated by myco hunger; L2/L6 ignore this region) -->

> 🫀 **Boot signals** (last `myco hunger`: `2026-04-15T00:55:02Z`)
>
> - [REFLEX HIGH] raw_backlog: 15 pure-raw notes (digest_count=0, non-exempt source) exceed threshold 10. IMMUTABLE REFLEX (W1 autopilot, con...
>
> Full brief: `.myco_state/boot_brief.md`

<!-- MYCO-BOOT-SIGNALS:END -->

**项目**：Myco，Agent-First 共生认知基质（框架本身）
**当前阶段**：v0.3.0 beta — PyPI 已发布，Show HN 待执行（尚未 1.0）；Post-Metamorphosis — Agent-First 有机体已建成。**Metamorphosis plan 完成**，kernel contract v0.46.0。关键里程碑：30 维 lint (L0-L28) + 25 MCP tools + 600 tests + Signal-to-Action hunger 自动执行闭环 + Link Graph 菌丝互联 + Semantic Cohort Intelligence + Inlet Trigger Policy + Continuous Compression + Session Memory FTS5 + **Wave 54 Agent-First Integration（hunger execute=true + scheduled metabolic cycle）**
...
6. **内化的变异-选择**：变异和选择都发生在 Agent+Organism 内部。Agent 产生变异（eat/digest/condense/absorb），Organism 的免疫系统执行选择（lint 30 维 + hunger signals + compression_pressure + dead_knowledge detection）。人类提供方向和信任，不需要做技术选择。透明性不可变——不是为了让人类选择，而是为了可审计性；没有审计能力 → 失去信任 → 系统被废弃。**永恒进化**（Perpetual Evolution）是三条不可变律之一：**停滞即死**——代谢循环停止运转的有机体会退化为静态缓存。
...
| 指标 | 值 | rationale | 证据锚 |
|------|----|-----------|--------|
| `v1_launch_progress` | 0.65 | PyPI v0.3.0 已发布；kernel contract v0.46.0；26 lint + Agent Review + 25 MCP tools + 600 tests；dist/ 1.1.0 wheel 已清理。剩余：README v1 里程碑对齐、Show HN launch | PyPI 0.3.0 live, Metamorphosis plan |
...
| `lint_coverage_confidence` | 0.85 | **30 维 L0-L28 全绿** + 600 tests（from 38; 2026-04-14: Wave D → 600 after doctor/diagnose merged into pulse）+ 29 modules + Waves A1-E3 新增 evolve.py/redact.py/project.py 全部测试 + Wave B 新增 symbionts adapters + hybrid lint（mechanical 28 dims + Agent Review 4 items）+ inline frontmatter validation on writes + hunger sidecar push-mode。上调至 0.85 | Metamorphosis plan commits + 600/600 pytest + 29/29 lint + Agent Review |
...
Myco 是一个 Agent-First 共生认知有机体框架。它给 AI agent 提供持久记忆、结构化知识和跨会话的自进化能力。v0.3.0 已发布至 PyPI（推荐：`git clone` + `pip install -e ".[mcp]"`；快速预览：`pip install myco`），提供 `myco seed / graft / immune / genome / assimilate / eat / correct / digest / evaluate / extract / integrate / observe / condense / expand / prune / absorb / hunger / forage / mycelium / colony / memory / pulse / propagate / rename` CLI。当前 kernel contract v0.46.0（30 维 lint (L0-L28) + Agent Review hybrid 免疫 + 25 MCP tools + 600 tests + hunger auto-execute + `myco seed --agent claude` 一键就位 + evolve.py 自我进化 + predict_knowledge_needs 需求预判）。
...
| 脚本 | 用途 |
|------|------|
| `scripts/lint_knowledge.py` | 30 维度自动化一致性检查（对照 `_canon.yaml`）；推荐通过 `myco immune` 调用 |
