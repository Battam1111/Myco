...
> 最后更新：2026-04-12
> **知识系统**：[Myco](https://github.com/Battam1111/Myco) v0.2.0（框架自用，pre-release；kernel contract v0.35.0）
...
> 🫀 **Boot signals** (last `myco hunger`: `2026-04-12T06:23:25Z`)
>
...
Myco 是一个可自进化的 AI 延伸认知基质框架。它给 AI agent 提供持久记忆、结构化知识和跨会话的自进化能力。v0.2.0 已可从源码安装（PyPI 公开发布准备中），提供 `myco init / migrate / lint / eat / evaluate / extract / integrate / digest / view / compress / uncompress / prune / inlet / hunger / forage / upstream / correct / molt` CLI。当前 kernel contract v0.35.0（含 Upstream Protocol v1.0 + Craft Protocol v1 + 23 维 lint L0-L22 + Self-Model D 层种子 + Metabolic Inlet MVP scaffold + Wave 43 全 Agent 表面 MCP 扩展：15 tools + Wave 46 Signal-to-Action wiring：HungerReport.actions 闭合 advisory→execution 缺口）。
