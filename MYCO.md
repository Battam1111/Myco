...
> 最后更新：2026-04-12
> **知识系统**：[Myco](https://github.com/Battam1111/Myco) v0.2.0（框架自用，pre-release；kernel contract v0.44.0）
...
> 🫀 **Boot signals** (last `myco hunger`: `2026-04-12T12:35:09Z`)
>
...
**当前阶段**：v0.2.0 pre-release — public launch 准备中（尚未 1.0）；Phase 2 消化道迭代进行中。**54 波落地**，kernel contract v0.44.0。关键里程碑：23 维 lint (L0-L22) + 18 MCP tools + 60 tests + Signal-to-Action hunger 自动执行闭环 + Link Graph 菌丝互联 + Semantic Cohort Intelligence + Inlet Trigger Policy + Continuous Compression + Session Memory FTS5 + **Wave 54 Agent-First Integration（hunger execute=true + scheduled metabolic cycle）**
...
Myco 是一个可自进化的 AI 延伸认知基质框架。它给 AI agent 提供持久记忆、结构化知识和跨会话的自进化能力。v0.2.0 已可从源码安装（PyPI 公开发布准备中），提供 `myco init / migrate / lint / eat / evaluate / extract / integrate / digest / view / compress / uncompress / prune / inlet / hunger / forage / upstream / correct / graph / cohort / session` CLI。当前 kernel contract v0.44.0（23 维 lint L0-L22 + 18 MCP tools + 60 tests + hunger auto-execute 自主代谢闭环 + Link Graph 菌丝互联 + Cohort Intelligence 智能分组 + Session Memory FTS5 + Scheduled Metabolic Cycle 每日自动代谢）。
