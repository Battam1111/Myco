...
> 🫀 **Boot signals** (last `myco hunger`: `2026-04-11T20:10:54Z`)
>
...
| 2 | 🔄 | v1.2 消化道 Phase ② ~~摩擦驱动~~ **doctrine-dependency-driven 迭代** ▰▰▰▰▰▱：Wave 26 re-audit 后改为自上而下由 8 锚点依赖图驱动 | **十五波落地（post Wave 8 re-baseline）**：Upstream v1.0 L12 (v0.8.0) + Craft Protocol v1 L13 (v0.9.0) + D 层 dead_knowledge 种子 (v0.17.0) + Boot/Session reflex (v0.12.0/v0.13.0) + L13 body schema (v0.14.0) + upstream scan writer/reader (v0.15.0/v0.16.0) + `myco correct` (v0.18.0) + silent-fail elimination + strict project-dir (v0.19.0) + L16 brief freshness + view agent surface (v0.20.0) + primordia compression W13 (v0.21.0) + hook dogfood + pytest gate (v0.22.0) + L17 contract drift (v0.23.0) + **tests/ 基础设施 (Wave 25, v0.24.0)** + **Wave 26 vision re-audit (v0.25.0)**。下一波：Wave 27 = 压缩原语 doctrine 对齐 craft（exploration 类） |
...
- **不变量**：所有摄取过的内容必须通过现有 **18 维 lint (L0-L17)**；任何降低 lint pass 率的摄取必须被拒绝
