# ASCC Agent Handoff Prompt — Myco contract v0.9.0 接入 / 升级指令

> **用途**：这是一条**可直接复制粘贴给 ASCC agent** 的完整任务指令，用于把
> ASCC 项目接入（首次）或升级（既有）到当前 Myco 基质 —— pre-release 包
> `myco 0.2.0`，kernel contract `v0.9.0`，23 维 lint（L0-L22），三通道代谢
> 架构 + 量化指标体系。
>
> **适用范围**：
...
>   解释为什么包版本从 1.1.0 降到 0.2.0、contract 从 v1.7.0 降到 v0.9.0）
...
=== BEGIN ASCC WAVE8 TASK: Myco contract v0.9.0 接入 / 升级 ===

你现在要完成 ASCC 项目对 Myco 基质当前版本 —— pre-release package `myco
0.2.0`，kernel contract `v0.9.0`，23 维 lint (L0-L22) —— 的接入或升级。
这是硬契约对齐任务，不是可选升级。
...
milestone。ASCC 侧可选择是否采纳 —— 不采纳也能通过 23 维 lint；采纳则会
在 Step 5.5 生成 ASCC 自己的指标面板。

## 背景（60 秒读完）

Myco kernel 截至 contract v0.9.0 已经落地的硬能力（每一条都会影响你接下来
在 ASCC 里能做什么）：
...
5. **三通道代谢**（contract v0.7.0+）：inbound (`forage/`) + internal
   ...
   outbound。
6. **量化指标体系**（contract v0.9.0，本 wave 新增）：`_canon.yaml::system
...
3. **`docs/contract_changelog.md` 的顶部 re-baseline banner + v0.9.0 条目**
...
减一的对应值：
    ...
    v1.6.0 → v0.6.0
    v1.7.0 → v0.7.0
    v0.8.0 → v0.9.0    # Wave 9 增量，若之前已同步到 v0.8.0 请继续升至 v0.9.0

然后升级到最新：

```yaml
  synced_contract_version: "v0.9.0"
```
...
**Wave 9 增量需要关注的两件事**（如果你是从 v0.8.0 升上来）：

1. **新 L10 source**：`upstream_absorbed` 现在是合法 note source；如果你以前
   因为碰到 L10 `Invalid source` 做过什么 hack，现在可以删除。
2. **新 substrate_key**：`upstream_inbox_pressure`（可选采纳；若 Step 5.5 采纳
   指标面板，就在面板里加一行，初值 0.00）。

**对 Cowork / Claude Code 用户的入口点提示（ce72 friction 响应）**：

如果 ASCC 运行在 Cowork 或 Claude Code 环境（agent 会自动加载 `CLAUDE.md`
而不是 `MYCO.md`），在 `myco init` 或迁移时使用：

```bash
myco init <project-name> --entry-point CLAUDE.md
```

这会把 `_canon.yaml::system.entry_point` 设为 `CLAUDE.md`，L1 / L11 / L14
等所有用到 entry_point 的 lint 都会自动指向 `CLAUDE.md`。不需要手动 override。
旧文档没有把这个 flag 写清楚；Wave 9 修了 discoverability，见
`docs/contract_changelog.md` v0.9.0 条目。
...
### Step 2 — 跑完整 23 维 lint baseline，看 ASCC 现状
...
```yaml
  ...
  synced_contract_version: "v0.9.0"   # last kernel contract aligned with
  ...
  # ── 量化指标体系 schema（contract v0.9.0，Wave 8 新增；可选采纳）─────
...
之后），粘贴以下块（如有旧版原地替换）：
    ...
    **🔒 Myco Hard Contract（contract v0.9.0，Wave 8 版）**
    ...
    **Session end sequence**：
    1. `myco lint --project-dir .` 确认 23/23 绿灯（L0-L22）
...
### Step 8 — 再跑一次完整 23 维 lint 验证

```bash
myco lint --project-dir .
```

期望结果：**L0-L22 共 23 维全绿**。如果仍有 HIGH/CRITICAL：
...
```bash
myco eat --content "ASCC Wave 8 接入 / 升级到 Myco contract v0.9.0 完成（2026-04-11）。变更：synced_contract_version v?.?.? → v0.9.0；write_surface Wave 8 版；notes_schema 含 optional fields；indicators 面板 <采纳 | 未采纳>；清理 <N> 个历史垃圾；Hard Contract 块已插入 MYCO.md 热区。L0-L22 lint 23/23 PASS。" --tags meta,migration,wave8,friction-phase2 --title "ASCC Wave 8 Migration Landed"
```
...
git commit -m "chore: align ASCC to Myco contract v0.9.0 (Wave 8)

- _canon.yaml: synced_contract_version -> v0.9.0; write_surface refreshed;
  notes_schema optional fields (view_count/last_viewed_at); indicators
  schema block <adopted | not adopted>
- MYCO.md: Hard Contract v0.9.0 block in hot zone (4 rules + boot/end);
  indicators dashboard section <added | n/a>
- Cleaned up <N> legacy scratch/TODO files (L11 baseline signal)
- L0-L22 lint 23/23 PASS

Ref: Myco kernel docs/agent_protocol.md, docs/craft_protocol.md,
docs/contract_changelog.md (v0.9.0 + Wave 8 re-baseline banner),
examples/ascc/migration_playbook.md"
```

**不要 push**。先让用户 review commit 再决定是否 push。

### Step 11.5 — Upstream 交付（仅当本次会话产生了 kernel friction bundle）

如果在 ASCC 运行过程中，你（作为 instance agent）向 `.myco_upstream_outbox/`
写入了任何 `n_*.bundle.(yaml|yml|json)` 文件（即命中了 Myco kernel 的契约
漏洞、lint 误报、模板缺口等），**不要自己删除 outbox**。这些 bundle 需要
kernel-side 主动收割（absorb），并在 kernel 侧落地为 `source: upstream_absorbed`
pointer note 后，才算被「消化」。

**交付路径（按优先级）：**

1. **Fast path — 同会话双挂载**：如果本次 Cowork 会话同时挂载了 Myco
   kernel 工作目录（通常在 `/sessions/*/mnt/Myco` 或用户显式选择），
   kernel-side agent 可以直接跑：

   ```bash
   myco upstream scan                              # kernel 侧扫自己 inbox
   myco upstream absorb <ascc_root_absolute_path>  # 从 ASCC 拉取 outbox
   myco upstream ingest <bundle_id>                # 落地为 pointer note
   ```

   `absorb` 会把 outbox 里的 bundle 原子复制到 kernel
   `.myco_upstream_inbox/`（按 `<absorb_ts>_<bundle_id>` 重命名去重）；
   `ingest` 会把单个 bundle 移入 `.myco_upstream_inbox/absorbed/` 并
   生成 `notes/n_*.md` 指针 note（`source: upstream_absorbed`, `tags`
   含 `upstream-bundle` / `source-project-ascc` / `<severity>-severity` /
   `kernel-<target_component>`）。

2. **Courier fallback — 跨会话 / 跨机器**：如果 kernel 不在同会话，
   用户需要手工把 `<ascc_root>/.myco_upstream_outbox/*.bundle.*`
   复制到 kernel 侧 `.myco_upstream_inbox/`（文件名前缀加
   `<YYYYMMDDTHHMMSS>_` 即可），然后由 kernel agent 跑 `myco upstream
   ingest <bundle_id>` 完成落地。详见 Myco kernel
   `docs/agent_protocol.md §8.5.2 Courier Fallback`。

**Instance 侧的职责（你这边）：**

- 保持 outbox 里的 bundle 原封不动 —— **不删除**，**不重命名**，**不压缩**。
- 在 Step 12 的报告里明列还挂在 outbox 的 bundle id + 每条的
  `target_kernel_component` 和 `severity`，让用户知道 kernel-side 还欠
  一次收割。
- 当下次会话开始时，kernel-side ingest 完成后会在 ASCC outbox 里留下
  一个「已被 absorb」的痕迹（可选：kernel agent 会 chore-commit ASCC
  侧的 outbox 清空），那时才可以把 outbox 里的 bundle 删除。

**不要做的：**

- ❌ 不要自己把 bundle 内容粘贴到 notes/ 里 —— L10 不允许
  `source: upstream_absorbed` 出现在 instance 侧（这个 source 是 kernel
  保留的）。
- ❌ 不要自己起 `myco upstream absorb` —— 那是 kernel 侧的动作，
  instance 侧跑只会什么都不发生。
- ❌ 不要在提交 commit 时把 `.myco_upstream_outbox/` 忘了；它需要被
  纳入版本控制，kernel agent 要通过 git/mount 看到它。

### Step 12 — 报告回用户

最后给用户一个完整的 Wave 8 接入报告，格式：

    ✅ ASCC Wave 8 Migration 完成（contract v0.9.0）

    变更摘要：
    - _canon.yaml:
        · synced_contract_version v?.?.? → v0.9.0
        · notes_schema: 含 Wave 8 optional fields
        · write_surface: Wave 8 allowlist
        · indicators: <采纳 | 未采纳>
    - MYCO.md:
        · 热区插入 Hard Contract v0.9.0 块（4 铁律 + boot/end）
        · <指标面板段已添加 | 未采纳>
    ...
    23 维 lint 验证：
      myco lint --project-dir . → 23/23 PASS（L0-L22 全绿）
    ...
    Upstream outbox（待 kernel 收割）：
      <列出 .myco_upstream_outbox/ 里每一条 bundle 的 id + severity
       + target_kernel_component；如果为空则写「无」>

    待用户决定：
      - 是否 push commit（现在是本地 staged）
      - 是否安排下次会话让 Myco kernel agent 跑
        `myco upstream absorb <ascc_root>` 收割上面那批 bundle（若有）
...
- `_canon.yaml` 含 Wave 8 版的 `synced_contract_version: "v0.9.0"` +
  `notes_schema` + `write_surface` 三个核心块；`indicators` 可选
- `MYCO.md` 热区含 Hard Contract v0.9.0 块
- `myco lint --project-dir .` → 23/23 PASS
