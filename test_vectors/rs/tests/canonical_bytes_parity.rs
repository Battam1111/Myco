...
//!
//! Same JSON is consumed by TypeScript (anchor-client) and Python
//! (kernel/governance). All three implementations must agree, or L1/HARD_RULES
...
fn vectors_path() -> PathBuf {
    // CARGO_MANIFEST_DIR = `<repo>/test_vectors/rs`; parent = `<repo>/test_vectors`
    let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    manifest_dir
        .parent()
        .expect("test_vectors/rs has a parent dir (= test_vectors/)")
...
fn all_canonical_bytes_vectors_decode_roundtrip() {
    ...
    // contract that Python and TypeScript decoders use (closes L1/HARD_RULES
