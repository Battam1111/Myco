...
schema}`. Owner-key custody is **not** here — that lives out-of-band in
[`anchor/`](../anchor/).
...
| **PERSISTENCE** | `persistence/` (`dag_io`, `manifest`, `snapshot`, `nonce_log`, `signing_key`, `operator_identity`), `persistence_health`, `persistence_runtime`, `at_rest_seal`, `backup`, `dpapi` *(win)*, `sealing` *(placeholder)* | On-disk DAG/state; at-rest sealing of the substrate secret; nonce log; backup. | SCHEMA · SKIN §4.2 |
