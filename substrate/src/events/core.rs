//! Core M21 substrate event vocabulary: genesis / cycle / axis, plus the
//! shared float-repr + hex-prefix utilities used by every domain.
//!
//! **v0.9 keyless**: the operator/owner_key codecs and the attestation-nonce
//! ledger codecs (nonce_issued / nonce_consumed / nonce_expired) were removed
//! with the owner-key + anchor surface. There is no owner key (substrate-ID is
//! self-derived per P01c) and no nonce-ledger issuance path.
...
// the owner_key_added / owner_key_archived codecs, AND the attestation-nonce
// ledger codecs (nonce_issued / nonce_consumed / nonce_expired node_types +
// encoders) were all removed here. There is no owner key and no nonce-issuance
// path in the keyless build.
