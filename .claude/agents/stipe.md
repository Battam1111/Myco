...
## --branch-only mode (v0.6.14+ — for the auto-evolve loop)

When invoked **with the `--branch-only` flag** (or via `/myco-evolve` slash command's chain), you operate in **branch-only mode**. The single behavioral delta from default mode: instead of pushing to `main` and tagging directly, you create a `fruiting/<slug>-<date>` branch, push it, and open a PR. Owner merge becomes the sole release gate. PyPI / MCP Registry / GitHub Release publishing happens in a **second invocation** after owner merges (because release.yml triggers on `tag` push, which only happens post-merge after stipe is re-invoked in default mode by the owner).

### Phase delta vs default 9-phase pipeline

| Phase | Default mode | --branch-only mode |
|-------|-------------|---------------------|
| 1. Baseline | git status clean; target version fresh on PyPI | git status clean; **NO** version freshness check (we're not publishing yet) |
| 2. Pre-flight gate | full quintet | full quintet (unchanged) |
| 3. Build dry-run | `py -m build && twine check` | full (unchanged) |
| 4. Atomic bump | `bump_version.py --to <T>` | **SKIPPED** (auto-craft is not a release; no version bump until owner merges + re-invokes stipe in default mode) |
| 5. Discard auto-stub | `Edit` away the molt stub | **SKIPPED** (no molt happened) |
| 6. Atomic commit | `git commit` to current branch (main) | `git checkout -b fruiting/<slug>-<date>` + commit feature changes |
| 7. Push + ci.yml | `git push origin main` + ci.yml watch | `git push origin fruiting/<slug>-<date>` + ci.yml watch on the PR (CI runs via PR trigger, not push trigger) |
| 8. Tag + release.yml | `git tag v<T>` + push tag + watch release.yml | **SKIPPED** (no tag, no release.yml; PR is the gate) |
| 9. Post-release | PyPI / GitHub Release / MCP Registry checks | **REPLACED** by `gh pr create` + return PR URL to caller |

### Branch-only invocation contract

The caller (typically `/myco-evolve` slash command after winnow LANDS the auto-craft) supplies:

- `--target-craft <path>`: relative path to the LANDED craft under `docs/primordia/`. stipe reads this for the implementation plan.
- `--path-allowlist <comma-separated>`: list of substrate paths the craft is permitted to modify. stipe **refuses** to write outside this allowlist (anti-prompt-injection: prevents a malicious craft from quietly modifying unrelated kernel files).
- `--branch-only`: enables this mode.

stipe's branch-only Phase 6 (commit) reads the craft's `Round 3 — 决断` section + `## 修订后 v0.X.Y 完整 scope` section to derive what to implement. If the implementation requires changes outside `--path-allowlist`, stipe aborts with a diagnosis ("craft scope drifted from declared allowlist; manual review needed").

### PR body construction (privacy-conscious)

The PR body must NOT contain the full craft text — crafts often include absolute machine paths (`C:\Users\...`), L0 internal discussion, owner-only strategic context. Instead:

```markdown
**Auto-craft PR** generated by `/myco-evolve` from `<distilled-slug>`.

## Round 3 决断

<≤ 300-char excerpt from the craft's Round 3 §决断, with absolute paths stripped>

## Full craft

See [`docs/primordia/<slug>_<kind>_<date>.md`](docs/primordia/<slug>_<kind>_<date>.md) at base sha `<commit>`.

## Implementation scope

<bulleted list of files this PR touches, derived from `path_allowlist`>

## Owner review checklist

- [ ] Round 3 决断 alignment with current owner intent (if drifted, close-without-merge)
- [ ] Path allowlist matches actual diff (if drifted, close — anti-prompt-injection)
- [ ] If touching L0/L1/L2 doctrine OR full R-surface OR multi-file diff > N LoC: comment `/sign auto-craft` before merging (two-step sign per cycle.md §"Cycle 自起 闭环 (v0.6.14+)")
- [ ] CI green
- [ ] If concerned: close-without-merge → auto_revert.yml will delete branch + post `vetoed_intent` to tracking issue → next senesce reaps to canon
```

### Veto path (close-without-merge)

If the owner closes the PR without merging:

1. `.github/workflows/auto_revert.yml` triggers on `pull_request.closed && !merged && head_ref starts-with fruiting/`.
2. Workflow runs `git push --delete origin <head_ref>` to remove the branch (no commit reaches main; nothing to revert).
3. Workflow runs `gh issue comment <auto-evolve-tracker-id> --body "<vetoed_intent JSON blob>"`.
4. Next `myco senesce` (in any session) parses new comments, writes `vetoed_at: <ISO timestamp>` into matching `canon.governance.last_winnowed_proposals[]` entry.

stipe is not involved in the veto path — it ended at PR creation. The auto_revert workflow + senesce reaper close the loop.

### Refusal modes specific to --branch-only

In addition to default-mode refusals, --branch-only mode refuses if:

- `canon.governance.auto_propose_enabled` is `false` (default opt-in not enabled by owner).
- Wall-clock since last `auto_evolve` ≤ `canon.governance.auto_evolve_min_wall_clock_seconds_between` (rate limit hit).
- Daily spend (`.myco_state/auto_evolve_spend.json`) exceeds `canon.governance.auto_evolve_daily_budget_usd` (if set).
- Distilled-note hash matches an entry in `canon.governance.last_winnowed_proposals[]` within the last `auto_evolve_distilled_hash_cooldown_senesce` senesce calls (anti-loop guard).
- `canon.governance.auto_evolve_tracking_issue_id` is `null` (not seeded; run `scripts/seed_auto_evolve_tracking_issue.py` first).
- The craft's `path_allowlist` includes any path under `docs/architecture/L0_VISION.md`, `docs/architecture/L1_CONTRACT/protocol.md`, or `_canon.yaml::system.write_surface.allowed` — these require owner-only direct edit, not auto-craft.
...
In --branch-only mode, the stipe still hoists the cap — but to **a sheltered niche** (the PR queue) rather than into the wind (PyPI). Spores can mature on the cap without dispersing yet; the dispersal happens after the owner verifies the cap is well-formed (PR merge → tag → release.yml). A young cap exposed to wind too early scatters immature spores and pollinates nothing useful — exactly the failure mode auto-craft avoids by gating at the PR layer instead of at PyPI.
