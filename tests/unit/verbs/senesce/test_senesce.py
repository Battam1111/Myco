...
the deprecation-shim import path (``from myco.cycle.session_end
...
def test_alias_session_end_no_longer_resolves() -> None:
    """v0.6.0 §A2 owner amendment: v0.5.2 alias 'session-end' REMOVED.

    Earlier (v0.5.3-v0.5.24) the contract was "alias resolves through
    v1.0.0". Round 4 owner directive accelerated removal to v0.6.0.
    """
...
def test_dispatch_via_session_end_alias_now_raises(genesis_substrate: Path) -> None:
    ...
    ctx = _mk_ctx(genesis_substrate)
    with _pytest.raises(UsageError):
        dispatch("session-end", {"quick": True}, ctx=ctx)
