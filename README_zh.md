...
**Cowork 用户**：`myco seed` 生成项目级配置，但 Cowork 不读取项目级 `.mcp.json`。要在 Cowork 里获得完整的 MCP 自动注入，请安装 `myco.plugin`，它把 MCP 服务器、技能和会话钩子打包成一个文件。详见 `docs/agent_protocol.md` §0.5。
