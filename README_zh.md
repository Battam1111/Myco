...
AI 记忆与知识系统的版图分为三个类别。Myco 自成一类。

| | 记忆即服务 | 知识编译 | 知识有机体 |
|---|---|---|---|
| **代表** | Mem0、Zep、LangMem | Karpathy LLM Wiki | **Myco** |
| **核心动作** | 存取 | 编译与索引 | **代谢** |
| **知识生命周期** | 无。数据进来、躺着、被查询。 | 仅摄入与 lint，无生命周期状态。 | 全管道：raw -> digesting -> extracted -> integrated -> excreted |
| **自我验证** | 无 | 基础矛盾/孤立检查 | 23 维免疫系统（L0-L22） |
| **自我进化** | 无 | 静态 schema | Craft 协议进化基质自身的规则 |
| **排出** | 永不。存储只增不减。 | 永不。 | 主动排出：死知识检测、自动剪枝、时间过期 |

**记忆服务负责存，知识编译器负责编，Myco 负责代谢。** 一个只进不出的系统不是活的知识库 —— 它是肿瘤。Myco 是唯一把知识视为拥有完整生命周期的系统：诞生、消化、验证、压缩，以及 —— 当它不再挣得自己的位置时 —— 被排出。生物学词汇不是装饰，它就是架构本身。
...
每个 LLM 系统都在进化**智能体的行为**；Myco 进化的是**智能体脚下的地面**。当某条 lint 规则不再抓到真问题，或者开始频繁误报，或者 canon schema 长出了自己 —— Myco 的代谢引擎会察觉并提出改动。四个代谢阶段持续运转：Hunger Sensing 在正常工作中感知摩擦；Session Reflection 在会话结束时反思基质本身该如何改进；Milestone Retrospective 在里程碑处挑战结构性假设；Cross-Project Distillation 在项目结束后把普适模式蒸馏回内核。除了这四个阶段，还有**代谢入口（metabolic inlet）** —— 负责把外部论文、代码仓、文章拉进来。进化的机制是**内化的变异-选择**：Agent 产生候选变更（mutation），Substrate 的免疫系统（lint + hunger）执行选择压力（selection）。
...
- **Toyota PDCA** —— Plan / Do / Check / Act 是自改进系统的基础循环。代谢引擎正是 PDCA 在 LLM 基质上的编译形态。
- **Voyager 技能库** —— 迭代式、基于 grounded 执行的技能积累是可能的，只要你记住什么曾经奏效并让下一集在它上面继续生长。这是 Cross-Project Distillation 蒸馏的形状。
...
那个令人不安却承重的真相：**我们早就在不自觉地运行这个系统的原始版本了。** 一个 8 天、80+ 文件的强化学习研究项目完整手工跑完了代谢循环。手动触发的 lint、口头的摩擦日志、人工驱动的元进化、15+ 次结构化辩论。它奏效了。然后在下一个项目上**又**奏效了。

<div align="center">

| 80+ 文件 | 10 个 wiki 页面 | 15+ 次结构化辩论 | 23/23 lint 维度全绿 |
|:--------:|:---------------:|:----------------:|:-------------------:|

</div>

Myco 是一个已经在野外自证过的模式的**形式化**。v0.x → v1.0 的路径不是"发明新东西"，而是"让靠手跑起来的东西开始自己跑"。通过 Cross-Project Distillation 提取出来的模式现在住在内核代码里。我们不是在请你相信一套理论，而是在请你相信一个已经在 80+ 文件、15+ 辩论的紧张研究项目里证明过自己的模式 —— 并帮我们拧紧那几个能让它在"人不当引擎"的情况下继续转的旋钮。
...
2. **触发信号。** 什么触发 Session Reflection？什么触发代谢入口？摩擦计数是个代理，真正的信号还是个开放研究问题。
...
| 阶段 | 已为真 | 即将到来 |
|------|--------|----------|
| **v0.x（今天）** | 四个向内齿轮已出货 · 23 维 lint 全绿 · 代谢 CLI 上线 · MCP 服务器暴露 19 个工具 · 内核/实例分离强制执行 · 无意识原型端到端验证 · 自我模型 D 层种子已落地（view 跟踪 + dead_knowledge 信号 + `myco prune` 自动排出）· 代谢入口 MVP scaffold 已出货（`myco inlet`，Wave 35 / v0.27.0）· L19 维度计数一致性 lint（Wave 38 / v0.29.0）· L20 翻译镜像一致性 lint（Wave 39 / v0.30.0）· L21 contract 版本内联一致性 lint（Wave 40 / v0.31.0）· L22 wave-seed 生命周期 lint（Wave 41 / v0.32.0） | 入口冷启动、自主触发信号、持续压缩仍开放。大多数齿轮触发仍是人工。 |
...
| `compress` | 将 N 条 raw/digesting 笔记合成为 1 条 extracted 笔记（含审计轨迹） | `myco compress` | `myco_compress` |
| `uncompress` | 逆转压缩，恢复输入笔记至压缩前状态 | `myco uncompress` | `myco_uncompress` |
| `prune` | D 层自动排出：扫描死知识笔记并打 excreted 标 | `myco prune` | `myco_prune` |
...
| `forage` | 管理外部参考材料（添加/列表/消化） | `myco forage` | `myco_forage` |
| `inlet` | 带来源追踪的外部内容摄入 | `myco inlet` | `myco_inlet` |
| `hunger` | 代谢仪表盘：raw 积压、陈旧笔记、死知识 | `myco hunger` | `myco_hunger` |
| `absorb` | 从下游实例同步内核改进 | `myco upstream absorb` | `myco_upstream` |
| `graph` | 链接图分析：反向链接、孤立文件、聚类、统计 | `myco graph` | `myco_graph` |
| `cohort` | 语义群组分析：标签共现、压缩建议、知识缺口 | `myco cohort` | `myco_cohort` |
| `session` | 会话记忆：Agent 对话索引·搜索·剪枝 | `myco session` | `myco_session` |
