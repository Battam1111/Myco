...
| 阶段 | 已为真 | 即将到来 |
|------|--------|----------|
| **v0.x（今天）** | 四个向内齿轮已出货 · 23 维 lint 全绿 · 代谢 CLI 上线 · MCP 服务器暴露 18 个工具 · 内核/实例分离强制执行 · 一个无意识原型（ASCC）端到端验证 · 自我模型 D 层种子已落地（view 跟踪 + dead_knowledge 信号 + `myco prune` 自动排出）· 代谢入口 MVP scaffold 已出货（`myco inlet`，Wave 35 / v0.27.0）· L19 维度计数一致性 lint（Wave 38 / v0.29.0）· L20 翻译镜像一致性 lint（Wave 39 / v0.30.0）· L21 contract 版本内联一致性 lint（Wave 40 / v0.31.0）· L22 wave-seed 生命周期 lint（Wave 41 / v0.32.0） | 入口冷启动、自主触发信号、持续压缩仍开放。大多数齿轮触发仍是人工。 |
...
**MCP 集成** —— 你的智能体自动获得 18 个工具，无需人工提示：
