...
**Claude Code / Cowork**：把本仓库的 `.claude/` 目录拷进你的项目——SessionStart 触发 `myco hunger`，PreCompact 触发 `myco session-end`。零手工仪式。

**任何 MCP host**（Cursor、Continue、Zed ⋯）：Myco 内置一个 manifest 驱动的 MCP 服务器：

```python
from myco.surface.mcp import build_server
build_server().run()          # 需要 pip install mcp
```

standalone `python -m myco.mcp` 启动器、`[mcp]` extras、官方 `.plugin` bundle 将在 **v0.4.1** 落地。
...
- **Claude Code / Cowork** —— 拷 `.claude/` 即可。官方 `.plugin` bundle 在 v0.4.1。
- **任何 MCP host** —— 把 `myco.surface.mcp:build_server()` wire 进 host 配置。
