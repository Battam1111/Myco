...
def _metabolic_skill_content(project_dir: str) -> str:
    ...
    return (
        ...
        f"4. Call myco_immune(project_dir=\"{project_dir}\") to verify substrate integrity — fix any issues found\n"
        f"5. Call myco_trace(entry_type='system', message='Metabolic cycle complete. Status: [healthy/N issues]', project_dir=\"{project_dir}\")\n"
        f"6. Self-verify: Run deployment health check via Bash: `cd \"{project_dir}\" && python -m myco verify --project-dir \"{project_dir}\"`\n"
        f"7. If verify finds issues (non-zero exit code), call myco_trace(entry_type='system', message='CRITICAL: Deployment verification failed: [list issues from verify output]', project_dir=\"{project_dir}\")\n"
...
def _generate_pre_commit_hook(project_dir: Path) -> bool:
    """Generate .git/hooks/pre-commit that runs ``myco lint --quick``.
    ...
    if hook_path.exists():
        ...
        if "myco" not in content:
            ...
            content += (
                ...
                "python -m myco lint --quick --project-dir . || exit 1\n"
            )
    ...
    hook_content = (
        ...
        "# Generated by `myco init`. Safe to customize.\n"
        "\n"
        "python -m myco lint --quick --project-dir .\n"
        "if [ $? -ne 0 ]; then\n"
        '    echo ""\n'
        '    echo "Myco lint failed. Fix issues before committing."\n'
        '    echo "   Run: myco lint --fix    (auto-repair mechanical issues)"\n'
        '    echo "   Run: myco lint          (see all issues)"\n'
        '    echo ""\n'
        "    exit 1\n"
        "fi\n"
    )
...
def init_level_2(project_dir: Path, replacements: dict, entry_point: str):
    ...
    print(f"  ✅ scripts/lint_knowledge.py (23-dimension Lint shim → myco.immune)")
