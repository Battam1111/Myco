...
def discover(home: Path) -> SymbiontProbe | None:
    """Probe ``home`` for an existing OpenClaw install; return SymbiontProbe or None."""
...
def install_basic(
    ...
    """Write the minimal Myco MCP entry to OpenClaw's config; honor dry_run."""
...
def install_deep(
    ...
    """Like install_basic plus OpenClaw-specific extras (none beyond MCP at v0.6.0)."""
    return InstallReport(host_id=HOST_ID, dry_run=dry_run)


def uninstall(probe: SymbiontProbe, *, dry_run: bool = False) -> UninstallReport:
    """Remove the Myco MCP entry from OpenClaw's config; honor dry_run."""
