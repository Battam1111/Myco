...
def _boot_with_owner_key(state: DispatcherState, tmp_path) -> Ed25519PrivateKey:  # type: ignore[no-untyped-def]
    """Hello handshake + init owner_keys with a known signing key's pubkey.

    Returns the private key so the test can sign CI mutation content (M10
    path: the dispatcher verifies attestation_signature against the active
    owner key when no reveal_pubkey is present)."""
    dispatch(
        state,
        Message(type=MessageType.HELLO, request_id=1, payload=hello_payload(b"\xaa" * 32)),
    )
    priv = Ed25519PrivateKey.from_seed(_SEED)
    pubkey = priv.public_key().bytes_
    dispatch(
        state,
        Message(
...
def _submit_migration(
    ...
    """Submit a schema_evolution mutation in migration_mode=true, signed by the
    owner key. Returns the decoded response fields."""
    sig = priv.sign(diff_bytes).bytes_
    response = dispatch(
        state,
        Message(
            ...
            payload=CbMap.from_dict(
                {
                    ...
                    "attestation_signature": CbBytes(sig),
...
def test_commit_migration_promotes_candidate_to_active(tmp_path) -> None:  # type: ignore[no-untyped-def]
    ...
    priv = _boot_with_owner_key(state, tmp_path)
    _register_axis(state, "hunger", 10.0)
    _submit_migration(state, priv, schema_diff_modify_axis_threshold_bytes("hunger", 77.0))
    assert state.candidate_gradient is not None
...
def test_abort_migration_drops_candidate_active_untouched(tmp_path) -> None:  # type: ignore[no-untyped-def]
    ...
    priv = _boot_with_owner_key(state, tmp_path)
    _register_axis(state, "hunger", 10.0)
    _submit_migration(state, priv, schema_diff_modify_axis_threshold_bytes("hunger", 55.0))
    assert state.candidate_gradient is not None
...
def test_commit_and_abort_are_idempotent_noops_without_candidate(tmp_path) -> None:  # type: ignore[no-untyped-def]
    state = DispatcherState()
    _boot_with_owner_key(state, tmp_path)
    assert state.candidate_gradient is None
...
def test_advance_equivalent_candidate_does_not_diverge(tmp_path) -> None:  # type: ignore[no-untyped-def]
    ...
    _submit_migration(state, priv, schema_diff_modify_axis_threshold_bytes("hunger", 99.0))
    fields = _advance(state, 1)
...
def test_advance_candidate_mortality_fruiting_diverges(tmp_path) -> None:  # type: ignore[no-untyped-def]
    ...
    _submit_migration(state, priv, diff)
    fields = _advance(state, 1)
    assert expect_bool(fields["candidate_active"]) is True
