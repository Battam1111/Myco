...
class MessageType(str, Enum):
    ...
    Closes L1/HARD_RULES C6 dag_enumeration_unclosed."""
...
class HmacMismatchError(BridgeProtocolError):
    ...
    Per L1/HARD_RULES C18 / C2 family: this is a CRITICAL skin breach in
...
def compute_hmac(body_bytes: bytes, key: bytes) -> bytes:
    ...
    """
    if len(key) == 0:
        raise BridgeProtocolError("HMAC key cannot be empty (per L1/SKIN §2)")
