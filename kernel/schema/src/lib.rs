...
//!
//! The substrate's state representation layer. Implements L1/SCHEMA §1-§4:
//!
...
//!   fields from L1/SCHEMA §3.1; child verifies closure (per I7 +
//!   L1/HARD_RULES C9 cold_resume invariants).
//! - **Validation tier dispatch** ([`validation`]) — per L1/SCHEMA §4: tier-1
...
//! - **L1/SCHEMA** §1 (SSoT), §2 (causal DAG), §3 (spore-schema),
//!   §4 (validation tiers)
...
//!
//! Logic + in-memory storage. Persistence to disk (per L1/SCHEMA §1.1 format
...
//! - Retro-edit detection via hash recomputation (closes L1/HARD_RULES C7
//!   for in-memory tampering; persistence-level detection lands in M2).
//!
//! **Deferred to M2+**:
//! - On-disk storage layout (L1/SCHEMA §1.1 leading candidates: YAML / TOML /
//!   JSON+JSONL / SQLite / custom binary+WAL). L4 picks.
//! - SSoT migration two-phase commit (L1/SCHEMA §1.3).
