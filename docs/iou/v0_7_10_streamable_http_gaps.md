...
> **Status**: OPEN. Surfaced 2026-05-10 by the v0.7.10 §G verification suite
> (`tests/integration/test_streamable_http_oauth.py`).
> **Layer**: L3 (gap log against L1 contract claims).
> **Scope**: blocks the README claim that `myco` ships an OAuth-2.1-protected
> streamable-HTTP MCP server. Boot-and-tools-list works; auth does not.
