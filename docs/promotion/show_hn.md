...
v0.5.24 shipped today, scoring A tier on Glama's Tool Definition Quality rubric. Running on PyPI; editable-default install is the recommended path (pip install -e).
...
- **First comment is not a repeat of the title.** Three numbered
  assumptions set up the philosophical claim before the concrete
  bullets deliver evidence. HN rewards structure-with-argument.
...
- **"v0.5.24 shipped today"** signals recency; active
