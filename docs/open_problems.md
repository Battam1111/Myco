...
> **相关**：`docs/current/vision_recovery_craft_2026-04-10.md` §4 "盲点列表"、
...
- `docs/current/` 的 COMPILED/SUPERSEDED 状态机（Craft Protocol v1.3.0）是手工迁移，
  理论上可以 lint 自动建议但未实现。
...
**最小可行种子**（Wave 4 计划）：
- `myco view` 写入 `notes/<id>.md` frontmatter 的 `last_viewed_at` + `view_count`。
- `myco hunger` 新增 `dead_knowledge` 信号：列出 `last_viewed_at > 30d ago && view_count < 2` 的 note。
- 这只是最小种子，不是 D 层完整实现——完整 D 层要求 view 语义、触发上下文、
  cross-reference 追踪，都还没设计。

**出口条件**：`myco view` 开始写 audit → `myco hunger` 暴露 dead_knowledge →
第一个真实的 excretion 决策基于 dead_knowledge 信号而非人工 → 升级为
instance_contract 级 craft。
...
- **移除**：仅当满足该条的 "出口条件" 时，把整节迁移到
  `docs/current/<problem_name>_resolved_YYYY-MM-DD.md`，然后在本文件留一行 stub
  指向 resolved 文件。
