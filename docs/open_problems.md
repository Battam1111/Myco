...
（default 5）且无 inlet 历史时触发，建议 `myco inlet` 引导。
...
任一触发 → hunger 报告包含 `inlet_ripe` advisory + `myco_cohort gaps` 路径。
...
**这些构成了第一条可操作的触发信号链（search miss → inlet_ripe → myco_cohort gaps → myco_inlet）。**
...
- `docs/primordia/` 的 COMPILED/SUPERSEDED 状态机（Craft Protocol）是手工迁移，
  理论上可以 lint 自动建议但未实现。
- Wave 30：`myco compress`（[`docs/primordia/archive/compress_mvp_craft_2026-04-12.md`](primordia/archive/compress_mvp_craft_2026-04-12.md)）forward compression verb。
- Wave 31：`myco uncompress`（[`docs/primordia/archive/uncompress_mvp_craft_2026-04-12.md`](primordia/archive/uncompress_mvp_craft_2026-04-12.md)）reverse compression。
...
- Wave 13: boot reflex arc（[`docs/primordia/archive/boot_reflex_arc_craft_2026-04-11.md`](primordia/archive/boot_reflex_arc_craft_2026-04-11.md)）—— 依赖 `myco_status` 被 MCP tool 调用。
