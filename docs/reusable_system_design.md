...
> **来源**：Myco 核心文档，ASCC 项目 Cross-Project Distillation 蒸馏写入（2026-04-09）
> **实战验证（ASCC v1）**：7 天，~80 文件，~13,500 行，12+ 次传统手艺辩论，首次 Milestone Retrospective 实战执行
...
system:                              # 跨项目通用
  ...
  lint_dimensions: 23               # L0-L22 (L12 removed) (current substrate)
...
```
Hunger Sensing (每次会话)          Session Reflection (每次会话结束)
  Friction Sensing     →     Session Reflection
  记录操作摩擦点              回答"系统哪里可以改进？"
       │                            │
       ▼                            ▼
Milestone Retrospective (每个里程碑)          Cross-Project Distillation (项目结束)
  Milestone Retrospective →  Cross-Project Distillation
  审视累积的 friction/meta      萃取通用模式，更新模板
  质疑架构假设 (Double-loop)
```

#### Hunger Sensing: Friction Sensing（摩擦感知）
...
#### Session Reflection: Session Reflection（会话反思）
...
#### Milestone Retrospective: Milestone Retrospective（里程碑回顾）
...
#### Cross-Project Distillation: Cross-Project Distillation（跨项目蒸馏）
...
- 发现缺少 Double-loop 机制 → v2 新增 Milestone Retrospective
- 发现 friction 记录门槛太高 → v2 新增 Hunger Sensing
...
[工作进行中]
  3. W1 即时沉淀（正常知识流动）
  4. Hunger Sensing: 遇到摩擦 → 记录 friction 条目

[会话结束]
  5. 标准结束清单（更新任务队列、log.md、进度）
  6. Session Reflection: 回答 meta-reflection 问题（1 句话）

[里程碑节点]
  7. Milestone Retrospective: 架构回顾（回答三个 Double-loop 问题）
  8. 必要时修改 WORKFLOW.md / CLAUDE.md 架构（需人类确认）

[项目结束]
  9. Cross-Project Distillation: 蒸馏通用模式，更新本模板
```
...
| 达到项目里程碑 | 执行 Milestone Retrospective 里程碑回顾 |
...
| 自动一致性检查 | ✅ Lint + SSoT | ❌ | ❌ | ❌ | ❌ |
| 深度决策机制 | ✅ 传统手艺 | ❌ | ❌ | ❌ | ❌ |
| 自进化引擎 | ✅ 四metabolic phase | ❌ | ❌ | ❌ | ⚠️ 技能库增长 |
...
| v2.0 | 2026-04-07 | +第四支柱（Timeline+Infra 独立）、+进化引擎（四metabolic phase）、一般化（去 ASCC 专属）、分级 Bootstrap、项目类型适配、_canon 分 system/project |
...
| v2.1+ | 2026-04-07 | **首次 Milestone Retrospective 实战** + v2.5 前置工具就绪。详见下方实战经验回写。 |

---

## ASCC 实战经验回写（Milestone Retrospective → 通用化）

> 以下发现来自 ASCC 项目首次 Milestone Retrospective 里程碑回顾（P4 中期），已泛化为通用建议。
...
ASCC 中 Hunger Sensing friction 条目 = 0（首次 Milestone Retrospective 发现时），说明引擎存在于文档中但未活在工作流中。原因：friction 触发门槛设得太高（"花费 > 预期 3 倍"），日常微摩擦全被过滤了。

**通用对策**：门槛降低至"差点犯错"和"需要多想一步"级别。宁可多记（一行成本可忽略），不可漏记（摩擦数据缺失导致 Milestone Retrospective 分析失效）。
...
- src/myco/immune.py -- 23 维 Lint 免疫系统实现（本文 §Pillar 4 的代码对应）
