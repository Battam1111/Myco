...
**Myco 是 Agent-First 的共生认知基质（Agent-First Symbiotic Cognitive Substrate）。**

这个定义由两个支柱构成：

**支柱一：延伸心智（The Extended Mind）**
...
这是 Myco 超越 Otto 笔记本、超越所有现有 agent 记忆系统的核心贡献。Myco 不只是存储，它通过四metabolic phase进化引擎持续改善自身：

```
知识代谢（Hunger Sensing/2）→ 架构质疑（Milestone Retrospective）→ 跨项目蒸馏（Cross-Project Distillation）
         ↑                                              │
         └──────────── 更新 bootstrap 模板 ←────────────┘
```

这对应 Argyris（1977）三环学习的工程实现：
- Hunger Sensing（摩擦感知）= 单环：修正具体行为
- Session Reflection（会话反思）= 双环：质疑底层假设
- Milestone Retrospective（里程碑回顾）= 双环在系统层：挑战架构假设
- Cross-Project Distillation（跨项目蒸馏）= **三环**：质疑"探究原则本身"，更新所有未来项目的起点
...
| **L4 进化** | 认知基质本身是否在改进？ | ❌ 架构固定 | ✅ Milestone Retrospective/4：架构自进化 + 跨项目蒸馏 |
...
每个新项目都从零开始积累操作经验。即使同一个 agent 在多个项目上工作，各项目的学习也是孤立的。Myco 的 Cross-Project Distillation 通过跨项目蒸馏，将个别项目的经验提炼为通用 bootstrap 模板，实现知识的**跨域迁移**。
...
| Hunger Sensing-4 四metabolic phase进化引擎 | Argyris & Schön（1978）双环/三环学习 | Hunger Sensing/2 = 单/双环；Milestone Retrospective = 系统层双环；Cross-Project Distillation = 三环 |
...
**C1（本质）**：Myco 是可自进化的 AI 共生认知基质，第一个在工程层面实现三环学习的 agent 认知外部化系统。
...
- [docs/evolution_engine.md](evolution_engine.md) -- 进化引擎 v3.0：代谢循环取代旧四齿轮，evolve.py 技能变异
- [docs/open_problems.md](open_problems.md) -- 未解难题：本文理论框架尚未解决的结构性盲点
- [docs/reusable_system_design.md](reusable_system_design.md) -- 通用架构与 Bootstrap 指南（Cross-Project Distillation 蒸馏产物）
- [_canon.yaml](../_canon.yaml) -- 规范值 SSoT：所有关键数字/常量的单一信息源
- [wiki/identity.md](../wiki/identity.md) -- Myco 身份定义：Agent-First Symbiotic Cognitive Substrate
