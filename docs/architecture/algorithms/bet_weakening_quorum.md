...
> Reference algorithm extracted from L0/cards/LB_living_bets.md §3 (falsifiability quorum) to keep active doctrine lean. Authoritative algorithm lives at **L2/OBSERVABILITY §3** (per-cycle detector) + L0/cards/LB_living_bets.md §3 (falsifiability quorum) (commitment); this file is the consolidated specification. The window length + 90-sample observatory_history cap are normative; the clock is the substrate's own (keyless v3.1.5; acknowledged-debt: no external trusted clock, see §1).

---

## §1. Algorithm (informal)

Over a **90-day window measured on the substrate's own clock** (keyless v3.1.5; acknowledged-debt: no external trusted clock. Was "anchor-stamped"; the substrate now reads its own wall-clock per L0/cards/P06_eternal_causality.md + L1/CONTINUITY (time semantics), and the window is a best-effort liveness horizon, not a security-bearing timeout):
...
- Window: 90 days on the substrate's own clock (keyless v3.1.5; acknowledged-debt: no external trusted clock). L1-tunable.
