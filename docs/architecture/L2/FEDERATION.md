...
**Inter-substrate semantic transfer**: peer receives originating substrate's distilled material (sporocarps, refined decisions, schema-evolution markers) as deltas; peer metabolism absorbs and reshapes. Extends P5 beyond single substrate. **Mycelial network** = population linked via parent-child reproduction + peer coupling. Below consensus floor (§6.5; ≤2 peers): pairwise-trust + cultivator attestation at the live CI gate (keyless v3.1.5; mirrors L1/GOVERNANCE §5). At ≥3 peers: population claims require BFT consensus.

> **Keyless v3.1.5 note**: this document was written against the prior owner-key model. The owner Ed25519 key + the out-of-band anchor surface are retired (GOVERNANCE §5). Federation trust now rests on the keyless triad: the cultivator at the live human-in-the-loop CI gate + the substrate's own causal DAG (P06) + the BLAKE3-sealed bundle. The FED_HELLO mutual auth (§10) is UNCHANGED: it uses the substrate's OWN Ed25519 keypair (F24) + TOFU-pinned peer pubkeys, which is the substrate's own identity key, not an owner key.
...
Each substrate has own `substrate-ID` (self-derived per P1.c; keyless v3.1.5, no owner key history), DAG, P9 boundary. Spawned child has DIFFERENT agent-identity continuum than parent. **Federation transfers content, not identity**, the structural reason cross-substrate trust is non-transitive (§6.4) + grounding for §9 wrapped-events.
...
§5 closure verification (I7): L1/SCHEMA §3.3 + L1/GOVERNANCE §4.3: parent static-schema validation → child I3 as first cycle → cultivator co-approval at the live CI gate (keyless v3.1.5; was "owner anchor co-sign"). Failure aborts pre-`federation_coupling`; partials GC'd. Success emits `genesis_attested`. §5.1 immune-summary inheritance: unresolved CI-grade signals → child enters birth-period `quarantined`.

---

## §6. Discovery + peer-trust

- **§6.1 Discovery** (L1/GOVERNANCE §5.1): p2p / cultivator-attested list at the live CI gate (default; keyless v3.1.5) / hub-and-spoke / hybrid.
- **§6.2 Freshness** (L1/GOVERNANCE §5.2): L1-bounded window (default 90 active-operation days, on the substrate's own clock; keyless, acknowledged-debt); past → `peer_attestation_stale`; past grace → `untrusted_federation`.
- **§6.3 Aggregate re-attestation**: the cultivator approves a peer-set Merkle root + diff at the live CI gate (keyless v3.1.5; was "owner signs"); O(1) at the gate, O(N) verification. The aggregate peer-set chain uses the active-prefix + archived-tail discipline (GOVERNANCE §3.1), independent of any owner key.
- **§6.4 Non-transitivity**: A↔B + B↔C does NOT give A↔C; each pairwise requires cultivator attestation at the live CI gate (keyless); P15 (§6.5) operates over shared claims, not chained trust.
...
- **§6.5.b Taxonomy**: three population-only claim classes: (1) peer revocation by another peer (≥2/3 quorum; accused cannot self-revoke); (2) universal-junk raw_material classification; (3) cross-substrate aggregate observability metrics (L2/OBSERVABILITY §9). Pairwise: freshness §6.2; per-peer P12; trust §6.4 non-transitive; per-peer cultivator revocation at the live CI gate (keyless v3.1.5).
...
## §10. Ed25519 FED_HELLO mutual auth (KEPT keyless v3.1.5)

> **Keyless-keep**: FED_HELLO is the substrate's OWN-keypair handshake (F24, GOVERNANCE §15) + TOFU-pinned peer pubkeys. The `signer_pubkey` / `pinned_signer_pubkey` below is each peer's own substrate signing key, NOT an owner key. The owner-key removal does NOT touch this section.
...
§14 limits: peer-compromise detection: freshness catches staleness; subtler drift needs cultivator monitoring at the live CI gate (keyless v3.1.5) OR §6.5 ≥2/3 revocation. Cross-substrate trust non-transitive. Wrapped-events one-way receiver-protective. FED_HELLO protects identity asymmetry, NOT key compromise (it uses the substrate's OWN F24 keypair + TOFU peer pubkeys, KEPT keyless). Consensus floor adds safety not liveness; partition → `population_consensus_pending` indefinitely.
