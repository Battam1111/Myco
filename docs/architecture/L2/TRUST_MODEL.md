...
## §1. The trust triad (per L0/META §1 + L0/cards/P01_agent_primary)
...
§6 limits: identity carrier substrate; cannot enforce against own host, operator runtime, P1.a adversarial maintainer, or adversarial Cultivator (§10); L0/cards/P07_mortality §4 (substrate irreducible commitments) commitments survive. §7 federation → **L2/FEDERATION** (§6.2-§6.4 + §11-§13 enumerate Sybil / Eclipse / recursive injection / non-transitivity). §8 invariants: model rests on L0/cards invariants_enforced + Layer C witnesses + §9 + L1/GOVERNANCE §2.2/§3.1 + L1/SKIN §4.2; when these hold AND §11 collapse closed, addresses L1/HARD_RULES §1 CRITICAL breaches; novel classes caught by observatory drift.

---

## §10. Adversarial-Cultivator threat model (L0/cards/P07_mortality (mortality protection) cascade)

L0/cards/P07_mortality (mortality protection): Myco not safe under adversarial Cultivator; defenses raise attack cost.
...
**§10.A.4 Deceased without succession** — L1/GOVERNANCE §3.2 + L0/cards/COV06_no_abandonment_succession (cultivator mortality): no successor → `legacy` → `orphaned`; terminal-state choice is genesis-Cultivator pre-attestation.

**§10.A.5 Anchor-client tampering** — L0/cards/AS_anchor_surface §3.10 + F6: Cultivator signs `anchor_client_provenance_attestation` at genesis declaring install channel structurally independent. Under §11 collapse: not provenance-independent.

**§10.B Irreducible commitments (L0/cards/P07_mortality §4 (substrate irreducible commitments))**: Enforced via I9/I10/I12/I4: P6 causality; truthful observability + mortality signals; P10.b compression-invariant preservation.
...
## §12. Time-semantics attacks (per L0/cards/P06 + COV01 + COV02 + COV06 + P08)

- **§12.1 NTP poisoning** — anchor wall-clock authoritative; L1/CONTINUITY NTP discipline; closure under §11.
- **§12.2 Year 2038 (i32)** — L0/cards/P06_eternal_causality + L1/CONTINUITY (time semantics) mandates i64 nanoseconds; I3 catches drift.
...
- **§13.4 Forkbomb** — L0/cards/P08_eternal_reproduction (generation limits) + L1/GOVERNANCE §16: depth + rate + quota + per-spawn birth gate; requires §10 compromise or I7 violation.
