...
### §1.1 Where the gate actually is: doctrine-time vs runtime (keyless honesty)

"The live human at the CI gate" means two different things at two layers, and being exact prevents reading in a guarantee that is not there:

- **Doctrine-time (the real gate).** Changes to the *doctrine* (the sealed L0 bundle and the rules around it) pass through the cultivator's review on this repository: a pull-request review, enforced mechanically by the BLAKE3 doctrine-bundle drift gate in CI (any edit to the sealed bundle that is not re-sealed fails the build). This is a genuine out-of-process human gate, and it is where "doctrine cannot silently drift from code" is actually enforced.
- **Runtime (no human daemon-gate).** Inside the running substrate there is no per-mutation human-approval round-trip. Ordinary runtime mutations are accepted under the operator-session channel; the bonded pilot-agent is the runtime authority. There is no owner key and no cryptographic gate interposed between the agent and the substrate.

This split is deliberate, not an omission. Under the single-human shared-fate model (同体共命; P01.c + CHAR07), the pilot is the cultivator's symbiote rather than an adversary, and the substrate runs entirely on the cultivator's own host. You do not defend the armor against your own bonded partner at runtime; you govern its *doctrine* at the repo gate, and you rely on the causal DAG plus the sealed bundle for after-the-fact auditability of what the runtime did. The residual risks of this choice (a compromised runtime session; an in-the-moment coerced human at the repo gate) are named at §10.A.2 / §10.A.3 + L0 D-0047, not papered over.
