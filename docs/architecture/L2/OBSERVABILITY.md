...
> **Scope**: canonical for Living Bets signals + falsifiability quorum. Cross-cuts L0 §7/§9.4/§14/§15 + L1/HARD_RULES §1 + I3/I5/I9/I10/I12 + L1/SCHEMA §2.4 + L1/CONTINUITY §1.2. All numeric thresholds L1-tunable unless specified.
...
Canonical: `algorithms/bet_weakening_quorum.md` + L0 §7.4. Window wall-clock 90d (anchor-stamped). Trend OLS `|slope/SE| ≥ Z=1.96`. Quorum ≥3 of 6 countable (5 until #4a lands). Spikes DAG-recorded but do not fire. Birth-period suspended. Bet retirement per L0 §7.5.

---

## §4. Per-cycle invariants (immune)

L1/CONTINUITY §1.1 + L1/SCHEMA §4 + L0 §6:
...
- **§4.3 Witnesses-not-verdicts** (L0 §9.3.4): emit crypto-proof tuples (sampled leaf hashes anchor-nonce-derived per §9.3.5, Merkle paths, parent hashes, check inputs). Substrate does NOT emit pass/fail. Sampling = `H(anchor_surface_nonce, leaf_count)`; substrate cannot bias. Extends to I9/I10/I12.
...
§8 doctrine (C37): **L0 §9.4** >10 CI events / 24h rolling wall-clock → `doctrine_instability_burst`. Counted: attestation acceptances; F1 classifier mutations; F3 owner-key rotations; L0/L1 revision diff records; F-row mutations. Rolling >12 months above → `doctrine_drift_grade`.
...
Cross-ref **L2/FEDERATION §13** for #4a/#4b + federation-specific signals. Health intra-substrate (own peers); network-level patterns L4-deferred. Per L0 §15: when federation ≥3 peers, P15 consensus-algorithm choice itself observable as P3 evolution event.
