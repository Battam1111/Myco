...
Genesis DAG contains only `genesis_event` + owner birth attestation. Early queries return ∅ or near-empty subgraphs. Substrate's "expressed direction" cold start IS agent's first few deltas (read directly from DAG, bypassing clustering per L1/CONTINUITY §6.2 birth-period). Queries MUST return explicit `cold_start_marker`, not error. Downstream consumers do NOT depend on trajectory being non-empty (appetite gradients are primary internal signal).
