...
| C5 | `attestation_invalid` (keyless local content-validation v3.1.5) | L1/GOVERNANCE §2.3 | Malformed CI envelope at the live CI gate — malformed `owner_objective` / compression / backup-status content (keyless: no owner-signature / nonce / dual-clock checks; the name + number are kept, the description reframed) | P1.b'' | I2 | **L** |
...
| C12 | ~~`successor_activation_with_fresh_owner_heartbeat`~~ | — | **RETIRED v3.1.5** (number reserved, not reissued): depended on the owner-liveness heartbeat, retired with the anchor surface. Keyless succession's only gate is the C46 catechumenate-session floor. | — | — | **retired** |
...
| C17 | ~~`operator_witness_forgery`~~ | — | **RETIRED v3.1.5** (number reserved): depended on the per-handshake REVEAL keypair envelope (`verify_reveal_keypair_envelope`), retired with the anchor surface. The keyless bestowal guarantee (substrate signs for itself) is carried by F24. | — | — | **retired** |
| C18 | `canonical_bytes_render_drift` (keyless v3.1.5) | L0/cards/P06 (F16 serializer) | Canonical-bytes render divergence — F16 determinism check (keyless: re-derived by the verifier at the live CI gate, not an owner-keyed anchor-client) | P1.b'' | I2 | **L** |
| C19 | `paused_dormancy_unsafe_host` | L1/CONTINUITY §2.4 + §3.2 | Process terminated during paused dormancy | P7, P1.c | I1 | **R** |
| C20 | ~~`genesis_attestation_chain_broken`~~ | — | **RETIRED v3.1.5** (number reserved): the owner-signed birth-attestation chain was retired with the anchor surface. Substrate-ID is now keyless (`hash(spore-schema-canonical-bytes, genesis-timestamp)`, F2); no owner-signed birth attestation to verify. | — | — | **retired** |

Rows INDEPENDENT (I2 fixed-point). Coverage: README §Status + L3/PACKAGE_MAP.md.

**Implementation notes** (rows now **L**):
- **C12** — **RETIRED v3.1.5** (was: owner-liveness-heartbeat-fresh successor gate). Removed with the anchor surface; keyless succession is gated solely by the C46 catechumenate-session floor (`substrate/src/cultivation.rs::handle_accept_succession`). Number reserved.
...
| C37 | `doctrine_instability_burst` | L1/GOVERNANCE §6 + L2/OBSERVABILITY §8 | P3 | I2, I4 | **L** |
...
| C41 | `dag_cb_integrity_violation` | L1/SCHEMA §2.1 + L0/cards/P06 (causal DAG integrity) | P1.c, P6, P10 | I4, I9 | **L** |
...
| C44 | ~~`nonce_substrate_minted_replay`~~ | — | — | — | **RETIRED v3.1.5** (number reserved): anchor-issued nonces were retired with the anchor surface; there are no anchor nonces for the substrate to mint/replay. |
| C45 | `substrate_id_low_entropy_collision` | L1/GOVERNANCE §4.1 + L0/cards/P01c §3.1 | P1.a | I1 | **U** |
| C46 | `owner_succession_bypass` (keyless catechumenate floor) | L1/GOVERNANCE §3.2 | P1.b'', P7 | I1, I2 | **L** |
...
| C50 | ~~`coerced_owner_suspected`~~ | — | — | — | **RETIRED v3.1.5** (number reserved): depended on the duress-keypair / signature-velocity machinery (F23), retired with the anchor surface. The keyless residual — a coerced human at the live CI gate — is named as acknowledged-debt (D-0047, L2/TRUST_MODEL §10.A.4); there is no cryptographic duress detector. |
...
| C70 | ~~`rotation_veto_window_violation`~~ | — | — | — | **RETIRED v3.1.5** (number reserved): the owner-key-rotation cooldown veto window was retired with the owner key (there is no owner key to rotate). |
...
- **C68** — promoted from §1.5; emit: `substrate/src/reproduction.rs::verify_spawn_co_attestation`; negative-witness: `substrate/tests/e2e_reproduction.rs::c68_unattested_spawn_refused_and_immune_no_child_dag` (+ siblings). The keyless C68 gate verifies the `myco-spawn-cosign-v1` envelope (parent replay-guard + spore-schema-hash binding + depth_override) — v3.1.5: the prior owner Ed25519 signature + owner-pubkey on the envelope are removed; the envelope-structure + replay gate is the kept gate. The §16.B rate breach (substrate-clocked) emits C48 on the same gate.
...
- **C70** — **RETIRED v3.1.5** (was: owner-key-rotation 30-day cooldown veto window; emit was `substrate/src/attestation.rs::rotate_owner_key`). Removed with the owner key (nothing to rotate); the `e2e_owner_key.rs` test file was deleted in the keyless teardown. Number reserved.
...
This row is **promoted into §1.2 above** (with its emit site + witnesses); the detailed gate-by-gate breakdown is kept here as the authoritative reference. Note on the C57-block: it is **partially populated, not wholly reserved** — beyond C61 + C66 (promoted to §1.2 above), several substrate-private integrity/health detectors are also implemented and firing (e.g. C57 `genesis_event_non_unique` / C58 `owner_pubkey_dag_pin_inconsistent` / C59 `manifest_cycle_vs_dag_advance_mismatch` in `substrate/src/integrity.rs` (C59 SUMS the spans of `cycle_advanced` nodes, Σ `new_cycle − prior_cycle`, not the node count, so a P04 §3.1 batch-summarized idle span of >1 still reconciles against `cycle_counter`); C62 `federation_event_oversized`; C64 `persistence_unavailable`; C65 `python_worker_slow_call`). Cataloguing the remaining C57-block rows into §1.2 with their full witness corpora is a v0.9.x housekeeping continuation; only the in-scope C61/C66/C68/C69/C70 are tabled in this pass. **C67 `doctrine_witness_drift`** is deliberately NOT a runtime row: it is a **CI-lint signal** raised by the `operators/claude` witness-lint (META §5.5) when a previously-failing negative witness silently starts passing — cards are doctrine, not substrate state (P01c), so the drift is caught at CI time and carries no `emit_immune_sporocarp` site.

## §2. Contract-identity fixed points (unconditionally CI)
L0-doctrinal; CI status unconditional; mutation REQUIRES cultivator approval at the live human-in-the-loop CI gate per L1/GOVERNANCE §2.2 (keyless v3.1.5: no owner-key signature).

> *(Keyless v3.1.5: **F3 `owner_key_history`**, **F4 `anchor_surface_endpoint_public_key`**, **F6 `anchor_client_provenance_attestation`**, and **F23 `duress_keypair`** are **DELETED** — all four were owner-key/anchor-surface fixed-points, retired with the anchor surface. The F-numbers are not reissued. **F2 / F5 / F16 / F24 are KEPT** keyless: F2 substrate-ID (now `hash(spore-schema-canonical-bytes, genesis-timestamp)`), F5 at-rest seal, F16 canonical-bytes serializer, F24 the substrate's own private signing keypair — F24 is NOT the owner key.)*

| # | Fixed-point | Defined at | L0 trace | I trace |
|---|---|---|---|---|
| F1 | Classifier dimension table + function | L1/GOVERNANCE §1.2 | P1.b'' | I2 |
| F2 | `substrate-ID` (immutable post-genesis; keyless `hash(spore-schema-canonical-bytes, genesis-timestamp)`) | L1/GOVERNANCE §4.1 | P1.a, P1.c | I1 |
| ~~F3~~ | ~~`owner_key_history`~~ — **DELETED v3.1.5** (owner key retired) | — | — | — |
| ~~F4~~ | ~~`anchor_surface_endpoint_public_key`~~ — **DELETED v3.1.5** (anchor surface retired) | — | — | — |
| F5 | `substrate_secret_sealing_mechanism_attestation` (at-rest seal; kept) | L1/SKIN §4.2 | P1.c | I1, I6 |
| ~~F6~~ | ~~`anchor_client_provenance_attestation`~~ — **DELETED v3.1.5** (anchor client retired) | — | — | — |
...
| F16 | `canonical_bytes_serializer_spec` (kept keyless) | L1/SCHEMA §3.1, §4.1 | P1.c, P6 | I1, I3, I4 |
...
| ~~F23~~ | ~~Duress_keypair registration~~ — **DELETED v3.1.5** (duress keypair retired with the anchor surface; residual coercion risk named as acknowledged-debt at D-0047 / L2/TRUST_MODEL §10.A.4) | — | — | — |
| F24 | Substrate-private signing keypair seed (KEPT — the substrate's OWN keypair; signs its own snapshot.cb + federation hello; NOT the owner key) | L1/GOVERNANCE §15.F24 | P1.c, P6 | I1, I4 |
...
## §4. Anchor-surface-resident state — RETIRED v3.1.5

**RETIRED with the anchor surface (v3.1.5 keyless teardown).** This section formerly enumerated owner-controlled, substrate-unauthorable state held on the out-of-band anchor surface (anchor nonces + consumed-nonce log; trusted wall-clock; liveness heartbeat; key-rotation cooldown veto; owner-signed successor attestations; anchor final-seal; anchor-client provenance; aggregate-reattestation Merkle commitments; L0/L1 revision diff co-sign records; DAG-tip co-signing logs; owner-signed substrate-ID birth attestation). **None of these exist in keyless mode.** What survives, keyless:

- **The peer revocation list (CRL)** is kept as ordinary substrate state, verified by the live human-in-the-loop at the CI gate (C13 egress block on revoked peers; not anchor-resident).
- **The generation-counter ceiling (F22)** is kept as a CI-class fixed-point (C47/C48 on breach).
- **The final-seal** is the BLAKE3 at-rest seal (F5) of the substrate's own state, not an anchor co-sign.
- **Doctrine-revision integrity** is the BLAKE3-sealed bundle (PROVENANCE §6) + a DAG event recorded at the live CI gate, not an owner-signed anchor diff record.

The trust root is now keyless (META §7.8): the live human-in-the-loop at the CI gate + the causal DAG + the BLAKE3-sealed bundle. The substrate still cannot author CI-class fixed-points on its own authority — a substrate-side forge attempt MUST be `untyped` (C14) / rejected at the CI gate — but the gate is a present human, not an owner key.
