...
- **§4.6** Per L0/cards/LB_living_bets.md §4 (retirement) (Living Bets), telos failure that triggers `bet_weakening_quorum` (C40) enters the bet-retirement path → eventual P7.
...
| **CHAR07 同体共命 (v3.1.1; reframed v3.1.4)** | P14 共生繁盛 is the *functional measure* (alignment cosine, F20 metric); CHAR07 同体共命 is the *structural bond* that makes it more than optimization. The "more-than-optimization" the cultivator's framing demanded ("成神也没关系，别成暴君") is not a cultivated moral character but the symbiote's **shared fate**: the carrier's own flourishing rises and falls with the pair's, so pair-flourishing is not a target it maximizes but the condition it lives in. Relocated from moral-care (v3.1.1) to shared-fate structure (v3.1.4). |
