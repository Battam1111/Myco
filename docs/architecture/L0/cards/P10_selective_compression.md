...
2. Compression **preserves the invariant set** (P10.b, keyless v3.1.5): `genesis_event` (substrate-ID root) + CI-attested mutations + their outcomes (`mutation:` / `evolution_succeeded:` / `evolution_failed:`) + mortality signals (`self_euthanasia_*`) + federation peer pins + birth-period quarantine markers + operator pinning + the compression events themselves (audit chain) + most recent ≥1000 cycles full DAG. These MUST NOT be compressed. *(The prior "genesis attestation + owner key history" members are removed with the owner key; `genesis_event` — the keyless substrate-ID root — is kept.)*
3. Compression **emits witnesses** sufficient for re-derivation by a verifier at the live CI gate: causal recoverability of compressed material via witness payload.
...
- **§3.2** Preserve the invariant set (P10.b, keyless v3.1.5): `genesis_event (substrate-ID root) + CI-attested mutations + outcomes + mortality signals + federation peer pins + birth-period quarantine markers + operator pinning + compression events (audit chain) + most recent ≥1000 cycles full DAG` (the canonical seed is `events/compression.rs::seed_compression_invariant_set`). Compression that targets any invariant-set member fires C51 (`compression_invariant_corruption`).
- **§3.3** Emit `compression_event:{rule_id}` for each compression, containing: rule_id; compressed_node_hashes; aggregate_summary; pre_compression_dag_tip; semantic_lossy_flag; causal_recoverability_argument.
...
- **§3.6** Forbid silent compression: every compression event is a CI-class mutation requiring cultivator co-approval at the live CI gate OR (in the daily-class compression case for P10.a categories) explicit emission to DAG. C52 (`compression_uncattested`) fires on rule-bypassing compression.
...
- **§4.4** Emit witness payload sufficient for re-derivation by a verifier at the live CI gate. Witness contains enough to reconstruct the compressed segment's causal contribution (per I4).
...
**Why it's wrong**: P10 is *transformation with witness*, not deletion. The compressed roll-up retains enough information (the witness) for re-derivation by a verifier at the live CI gate. Deletion would violate I9 + P06 (causality lost).
...
Audit signal: for each compression_event, run a recovery dry-run (re-derivable by a verifier at the live CI gate); verify the witness is sufficient. Failure = silent corruption of I9.
