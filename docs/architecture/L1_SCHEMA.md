...
**§4.3 Rolling-window coverage enforcement**: Tier-2 MUST achieve 100% coverage per window. Non-empty `missed_field_set` → `tier_2_coverage_breach`; ≥3 consecutive → quarantine (L1_CONTINUITY §5.1).
...
**§6.3 Verification (boot path)**: (1) Format check; failure → Ok(None) + full DAG replay. (2) Signer-pubkey identity MUST match substrate's derived pubkey (§7); failure → emit `C38_snapshot_integrity_violation`; discard + full replay. (3) Signature verification; failure → emit C38; discard + full replay. Snapshot AND DAG replay both fail → quarantine entry per L1_CONTINUITY §5.1.
...
Full catalog at L1_HARD_RULES §1: C6 (§2.2), C7 (§2.1), C8 (§1.3), C18 (§5), C38 (§6), C51 / C52 (§2.5), C53 (§4.1).
