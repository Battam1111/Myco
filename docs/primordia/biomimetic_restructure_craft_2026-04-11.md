...
- 本次 craft **不承诺具体合并哪几份**。只承诺：执行时用上述判据逐对评估。
  ...
  1. `ascc_migration_v1_2.md` + `ascc_agent_handoff_prompt.md` →
     ...
     但信息 85% 重叠。合并到 `docs/instance_migration.md`，保留两节区分受众。
     **风险：低**。
...
- ✅ 合并两份 ASCC 文档 → `docs/instance_migration.md`
...
- `docs/ascc_agent_handoff_prompt.md` + `docs/ascc_migration_v1_2.md`: 4 处
...
1. `wc -l docs/ascc_migration_v1_2.md docs/ascc_agent_handoff_prompt.md`
2. 读两份文件，逐节对比，量化 overlap
3. **Gate**：overlap ≥ 70% → 继续合并；< 70% → 跳过，更新 biomimetic_map 记录
4. 若合并：创建 `docs/instance_migration.md`，保留两份文件原有的两个 audience
