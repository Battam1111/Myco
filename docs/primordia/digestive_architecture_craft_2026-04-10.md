...
> **时间框架说明**：本 craft 原本用 Week 1/2/3 作阶段标识，2026-04-10 Yanjun 反馈"我们的推进速度由摩擦决定，不是日历决定"后改为 Phase ①/②/③ 进度条。每个 Phase 的进入与完成由**硬验收指标**而非日历周数决定——Phase ① 验收通过才开 Phase ②，Phase ② 收敛到稳定才开 Phase ③。硬指标低于 50% 即回炉，不看投入了多少时间。
...
**下一步**：Yanjun 批准后即开始 Phase ① 实现（独立工作单元，不阻塞本 debate 归档）。实施节奏由 Gear 1 摩擦信号驱动，不绑定任何日历窗口。
