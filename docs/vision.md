...
> **定位**：Agent-First 共生认知基质（Symbiotic Cognitive Substrate）
...
**一句话**：Agent-First 共生认知基质，Agent 的另一半。它记忆、验证、消化、进化，让 Agent 不再每次从零开始。
...
**为什么是"基质"而非"基础设施"**：基础设施是被动的服务提供者（路网、水管、数据库），暗示"被使用"；基质是生物膜附着与代谢的介质，是酶与底物的共生空间，是土壤般的生成性母体。Myco 的产物同时是它的组件（lint 约束 lint，canon 约束 canon），这是 autopoiesis 意义上的自生成系统。详见 `docs/primordia/substrate_identity_upgrade_craft_2026-04-14.md`。
...
**我们的独特定位**："第一个能自我进化的共生认知基质"，Agent+Substrate 共生体自主运转代谢循环（26 维 lint + hunger 自动执行 + 每日 scheduled task），人类只需用自然语言提供方向。
...
- **Substrate 执行选择**：lint 26 维 + hunger signals + compression_pressure + dead_knowledge detection 构成内部免疫系统，过滤无效变异
- **人类提供方向**：用自然语言表达意图（"我想要 X"），不需要懂技术细节
- **透明性保障信任**：所有行动可审计（log.md, notes/, craft），人类随时可以看，但不需要看

> 关于 mutation-selection 模型（v0.40 anchor revision）：早期表述为"系统做变异，人类做选择"，现已内化。生物体不会等外部选择每次细胞分裂，免疫系统在内部选择。Myco 的 lint + hunger 就是这个免疫系统。
...
- [docs/primordia/substrate_identity_upgrade_craft_2026-04-14.md](primordia/substrate_identity_upgrade_craft_2026-04-14.md)：基质身份升级 Craft
- [wiki/identity.md](../wiki/identity.md)：Myco 身份定义，Agent-First Symbiotic Cognitive Substrate + 七条生命标准 + 四项结构特征
