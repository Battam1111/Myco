...
```
## [2026-04-09] friction | [操作摩擦] badge 被 axes ylim 截断，坐标补丁修两轮仍复发 → g4-candidate
```
...
1. **萃取**（Extraction）
   ...
   - 示例：
     - ✅ "四齿轮自进化机制"（通用）→ 本文件
     - ✅ "摩擦感知触发类别"（通用）→ 本文件
     - ✅ "三层 axes 布局模式"（通用）→ research_paper_craft.md
     - ❌ "算法消融设计"（特定）→ 保留在项目 wiki
     - ❌ "大规模实验调度器"（特定）→ 存档，不通用化
...
**项目中的实际程序**：
```
## [2026-04-07] meta | 固定参数值导致效果下降，"固定参数足够"的假设被证伪；触发 Gear 3

Gear 3 Q1：假设"固定参数足够"被证伪 ✓
Gear 3 Q2：wiki/experiment_design 中消融组设计过时 ✓
Gear 3 Q3：Agent 权限不足以快速迭代参数，需要扩权 ✓
结果：新的消融实验启动，权限扩展至 Agent 可独立调整实验参数
```
