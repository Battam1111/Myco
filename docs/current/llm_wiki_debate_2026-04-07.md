...
| Karpathy | 我们的系统 | 对应程度 |
|----------|-----------|---------|
| **Raw Sources**（不可变原始资料） | L0 会话历史（JSONL）+ docs/current/（辩论记录） | 部分。我们的"原始资料"是会话对话，不是外部文献 |
...
| 操作 | Karpathy | 我们 | 差距 |
|------|---------|------|------|
| **Ingest** | 新文档 → 更新多个 wiki 页面 | 传统手艺 → 写入 CLAUDE.md + docs/current/ | 有 ingest，但写入位置靠惯例非协议 |
