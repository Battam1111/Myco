...
test("InMemorySealedKey: pubkey + sign round-trip", () => {
  ...
  // The signature should verify with the corresponding pubkey, and the
  // sealed key's pubkey must match what Ed25519PrivateKey derives from the
  // same seed (otherwise the "sealed" wrapper has drifted from the underlying
  // primitive).
  const expected_priv = Ed25519PrivateKey.fromSeed(seed);
  const expected_pub = expected_priv.publicKey();
  const expected_sig = expected_priv.sign(msg);
  assert.equal(pub.toHex(), expected_pub.toHex());
