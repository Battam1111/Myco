...
// gap where an adversary's blob is rendered by anchor-client but rejected
// by the substrate.
...
// ---------------------------------------------------------------------------
// L1/HARD_RULES C18 detection: two render passes on identical canonical bytes
// must produce identical text.
// ---------------------------------------------------------------------------

test("L1/HARD_RULES C18: identical canonical bytes → identical render", () => {
  const v: Value = {
    type: "map",
    value: new Map([
